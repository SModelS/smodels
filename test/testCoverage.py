...
st1 = mssm.st1
st1.totalwidth = 2.0 * GeV
st2 = mssm.st2
st2.totalwidth = 10 ** (-15) * GeV
gluino = mssm.gluino
gluino.totalwidth = 1.0 * 10 ** (-30) * GeV
t = SMparticles.t
b = SMparticles.b
...
filters = {
    "missing MET (prompt)": lambda el: (not ("prompt" in el.coveredBy))
    and (el.branches[0].oddParticles[-1].isMET() and el.branches[1].oddParticles[-1].isMET()),
    "missing non-MET (prompt)": lambda el: (not ("prompt" in el.coveredBy))
    and (
        not el.branches[0].oddParticles[-1].isMET() or not el.branches[1].oddParticles[-1].isMET()
    ),
...
for key in filters:
    if "prompt" in key.lower():
        ...
        factors[key] = lambda el: reweightFactorFor(el, "prompt") + reweightFactorFor(
            el, "displaced"
        )
...
class CoverageTest(unittest.TestCase):
    def testUncovered(self):
        ...
        self.assertAlmostEqual(longLived.generalElements[0].missingX, 10.0)
        self.assertAlmostEqual(displaced.generalElements[0].missingX, 9.96109334317542, places=4)
        self.assertAlmostEqual(MET.generalElements[0].missingX, 10.0)
    ...
    def testUncoveredTree(self):
        ...
        p2 = Particle(
            ...
            eCharge=0.0,
            colordim=1,
            totalwidth=1e-15 * GeV,
        )
        xsecA = XSection()
        xsecA.value = 10.0 * fb
        xsecA.info.sqrts = 13.0 * TeV
        xsecA.info.label = "wA"
        xsecA.info.order = 0
        xsecB = XSection()
        xsecB.value = 5.0 * fb
        xsecB.info.sqrts = 13.0 * TeV
        xsecB.info.label = "wb"
        xsecB.info.order = 0
        ...
        self.assertEqual(
            len(missingTopos.generalElements), 1
        )  # Only elCombined should appear (it is smaller than a1)
        self.assertAlmostEqual(
            missingTopos.generalElements[0].missingX, 5.0
        )  # Only the fraction of the cross-section from elB is not missing
        self.assertEqual(
            len(missingTopos.generalElements[0]._contributingElements), 1
        )  # Only elCombined should appear
