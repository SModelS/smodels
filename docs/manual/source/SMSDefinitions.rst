...
The :ref:`string representation <notation>` of a |SMS| can be quite lengthy and it is
not always fitting for some types of output. Therefore it is convenient to introduced a 
shortened version of its string representation, which includes only the |SMS| final states, i.e. the undecayed particles in the topology.
For readability, the final states are group according to their primary mother produced in the topology.
For instance, for a |SMS| described by: ::

   PV > gluino(1),su_L(2), (gluino(1) > N1(3),q(4),q(5)), (su_L(2) > q(6),N1(7))

the corresponding final state representation is given by: ::

   PV > (q,q,N1),(q,N1)

Although the final state representation does not uniquely identify the |SMS|, it is more compact and still provides information about the type of final state generated by the topology.
