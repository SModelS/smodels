...
RHadronU = Particle(label="RHadronU", Z2parity=-1, eCharge=2.0 / 3.0, colordim=3)
RHadronD = Particle(label="RHadronD", Z2parity=-1, eCharge=-1.0 / 3.0, colordim=3)
RHadronQ = MultiParticle(
    label="RHadronQ",
    particles=[RHadronU, RHadronU.chargeConjugate(), RHadronD, RHadronD.chargeConjugate()],
)
...
for key in filtersDefault:
    if "prompt" in key.lower():
        ...
        factorsDefault[key] = lambda el: reweightFactorFor(el, "prompt") + reweightFactorFor(
            el, "displaced"
        )
...
class Uncovered(object):
    ...
    def __init__(
        ...
        if not isinstance(groupFilters, dict):
            raise SModelSError(
                "groupFilters input should be a Dictionary and not %s" % type(groupFilters)
            )
        if not isinstance(groupFactors, dict):
            raise SModelSError(
                "groupFactors input should be a Dictionary and not %s" % type(groupFactors)
            )
...
class UncoveredGroup(object):
    ...
    def getToposFrom(self, topoList):
        ...
        missingXandEls = [
            [self.getMissingX(el) * self.reweightFactor(el), el] for el in elementList
        ]
        # Only keep the ones elements sigmacut:
        if self.sigmacut:
            missingXandEls = [x for x in missingXandEls[:] if x[0] > self.sigmacut]
        else:
            missingXandEls = [x for x in missingXandEls[:] if x[0] > 0.0]
        # Sort according to largest missingX, smallest size and largest ID
        missingXandEls = sorted(
            missingXandEls, key=lambda pt: [pt[0], -pt[1]._getLength(), -pt[1].elID], reverse=True
        )
        ...
        self.generalElements = sorted(
            self.generalElements[:], key=lambda genEl: genEl.missingX, reverse=True
        )
