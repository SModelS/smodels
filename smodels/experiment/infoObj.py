...
class Info(object):
    ...
    def cacheOnnx ( self ):
        """ if we have the "onnxFile" attribute defined,
            we cache the corresponding onnx file. Needed when pickling """
        if not hasattr ( self, "onnxFile" ):
            return
        ...
        with open(pathName, "r") as f:
            try:
                ...
                logger.error ( f"cannot load {pathName}: {e}" )
                raise(e)
