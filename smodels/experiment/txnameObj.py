...
from smodels.theory.auxiliaryFunctions import (
    elementsInStr,
    removeUnits,
    unscaleWidth,
    rescaleWidth,
...
unitsDict = dict(
    [
        [varname, varobj]
        for varname, varobj in physicsUnits.__dict__.items()
...
class TxName(object):
    ...
    def __init__(self, path, globalObj, infoObj, databaseParticles):
        ...
        for i, tag in enumerate(tags):
            ...
            if tags.count(tag) != 1:
                logger.info("Duplicated field %s found in file %s" % (tag, self.path))
            if ";" in value:
                value = value.split(";")
            if tag == "upperLimits":
                data = value
        ...
        self.txnameData = TxNameData(
            data, dataType, ident, Leff_inner=self.Leff_inner, Leff_outer=self.Leff_outer
        )
        if expectedData:
            self.txnameDataExp = TxNameData(
                expectedData,
                dataType,
                ident,
        ...
        if hasattr(self, "constraint"):
            elements += [
                Element(el, self.finalState, self.intermediateState, databaseParticles)
        ...
        if hasattr(self, "condition") and self.condition:
            ...
            for cond in conds:
                for el in elementsInStr(str(cond)):
                    newEl = Element(el, self.finalState, self.intermediateState, databaseParticles)
    ...
    def getEfficiencyFor(self, element):
        ...
        if self.txnameData.dataType == "efficiencyMap":
            ...
            if not eff or math.isnan(eff):
                eff = 0.0  # Element is outside the grid or has zero efficiency
        elif self.txnameData.dataType == "upperLimit":
            if hasattr(self, "dbClient"):
                query = self.getQueryStringForElement(element)
                logger.info(
                    "sending query %s to %s:%d"
                    % (query, self.dbClient.servername, self.dbClient.port)
                )
                # print ( "query will be", query )
                # return 0.001
            ...
            if ul is None:
                eff = (
                    0.0  # Element is outside the grid or the decays do not correspond to the txname
                )
            else:
                eff = 1.0
        else:
...
class TxNameData(object):
    ...
    def __init__(
        self, value, dataType, Id, accept_errors_upto=0.05, Leff_inner=None, Leff_outer=None
    ):
        """
        ...
        if self.dataType == "efficiencyMap":
            ...
            raise SModelSError(
                "Default reweighting function not defined for data type %s" % self.dataType
            )
    ...
    def getUnits(self, value):
        ...
        if isinstance(value, tuple):
            ...
            return dict([[self.getUnits(x), self.getUnits(y)] for x, y in value.items()])
            ...
            for unit in stdUnits:
                ...
                if not y._unit:
                    return unit
            raise SModelSError(
                "Could not find standard unit which matches %s. Using the standard units: %s"
    ...
    def dataToCoordinates(self, dataPoint, rotMatrix=None, transVector=None):
        ...
        widths = [
            [widths[ibr][im] for im, _ in enumerate(br) if (ibr, im) in self.widthPosition]
    ...
    def coordinatesToData(self, point, rotMatrix=None, transVector=None):
        ...
        if len(point) != self.full_dimensionality and len(point) != self.dimensionality:
            ...
            pointFull = np.append(pointFull, [0.0] * (self.full_dimensionality - len(point)))
        else:
            pointFull = np.array(point[:])
        ...
        massUnit = [
            unit for unit in physicsUnits.standardUnits if not (1 * GeV / unit).normalize()._unit
        ][0]
        ...
        for ibr, br in enumerate(massArray):
            if str(br) != "*":
                newBr = [
                    (m, widths.pop(0)) if (ibr, im) in self.widthPosition else m
                    for im, m in enumerate(br)
                ]
            else:
                newBr = br
    ...
    def loadData(self, value):
        ...
        self.dataShape = self.getDataShape(
            val[0][0]
        )  # Store the data (mass) format (useful if there are inclusives)
        self.widthPosition = self.getWidthPosition(
            val[0][0]
        )  # Store the position of the required widths
        values = removeUnits(
            val, physicsUnits.standardUnits
        )  # Remove units and store the normalization units

        if len(values) < 1 or len(values[0]) < 2:
            raise SModelSError(
                "input value not in correct format. expecting sth "
    ...
    def getValueFor(self, element):
                ...
                for iw, w in enumerate(br):
                    if (ibr, iw) in self.widthPosition:
                        if iw != len(br) - 1:
                            ...
                            widths[ibr].append(0.0 * GeV)
                    else:
                        widths[ibr].append(w)
            reweightFactor = self.reweightF(
                widths, Leff_inner=self.Leff_inner, Leff_outer=self.Leff_outer
            )
        ...
        point = self.dataToCoordinates(element, rotMatrix=self._V, transVector=self.delta_x)
    ...
    def getValueForPoint(self, point):
        ...
        if dp > self.dimensionality:  ## we have data in different dimensions
            if self._accept_errors_upto == None:
                return None
            logger.debug(
                "attempting to interpolate outside of convex hull "
                "(d=%d,dp=%d,point=%s)" % (self.dimensionality, dp, str(point))
            )
    ...
    def interpolate(self, point, fill_value=np.nan):
        ...
        wts = np.append(bary, 1.0 - bary.sum())
        # Vertex indices:
        ...
        if ret < minXsec:
            logger.debug(
                "Interpolation below simplex values. Will take the smallest simplex value."
            )
            ret = minXsec
        return float(ret)
    ...
    def _estimateExtrapolationError(self, point):
        ...
        alpha = float(np.sqrt(np.dot(point[self.dimensionality :], point[self.dimensionality :])))
        if alpha == 0.0:
    ...
    def _returnProjectedValue(self):
        ...
        if abs(self.projected_value) < 100.0 * sys.float_info.epsilon:
    ...
    def onlyZeroValues(self):
        ...
        if negative_values:
            for x in self.y_values:
                if x < -eps:
                    logger.error("negative error in result: %f, %s" % (x, self._id))
                    sys.exit()
        if sum(self.y_values) > 0.0:
            return False
        return True
...
class Delaunay1D:
    ...
    def __init__(self, data):
        ...
        if data and self.checkData(data):
            ...
            self.simplices = np.array(
                [
                    [data.index(self.points[i + 1]), data.index(pt)]
            ...
            for simplex in self.simplices:
                ...
                transform.append([[1.0 / (xmax - xmin)], [xmin]])
    ...
    def find_simplex(self, x, tol=0.0):
        """
    ...
    def checkData(self, data):
        ...
        for pt in data:
            if (not isinstance(pt, list)) or len(pt) != 1 or (not isinstance(pt[0], float)):
                logger.error(
                    "Input data for 1D Delaunay is in wrong format. It should be [[x1],[x2],..]"
                )
                return False
        return True
...
if __name__ == "__main__":
    ...
    t0 = time.time()
    for masses in [
        [[302.0 * GeV, 123.0 * GeV], [302.0 * GeV, 123.0 * GeV]],
        [[254.0 * GeV, 171.0 * GeV], [254.0 * GeV, 170.0 * GeV]],
    ]:
    ...
    print("%.2f ms" % ((time.time() - t0) * 1000.0))
