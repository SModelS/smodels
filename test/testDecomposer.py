...
class DecomposerTest(unittest.TestCase):
    def testDecomposerLHE(self):
        ...
        self.assertAlmostEqual(element.mass, [[675.0 * GeV, 200.0 * GeV]] * 2)
        xsec = [xsec for xsec in element.weight if xsec.info.sqrts == 8.0 * TeV][0]
    ...
    def testDecomposerLongLived(self):
        ...
        newModel = [
            ptc
            for ptc in BSMList
            if not isinstance(ptc.pdg, list) and abs(ptc.pdg) in [1000015, 1000022]
        ]
        ...
        expectedWeights = {
            str(sorted([["N1"], ["N1"]])).replace(" ", ""): 0.020,
        ...
        for el in topList.getElements():
            ...
            xsec = el.weight.getXsecsFor(8.0 * TeV)[0].value.asNumber(fb)
    ...
    def testCompression(self):
        ...
        model.updateParticles(
            filename, promptWidth=1e-12 * GeV
        )  # Force charginos/neutralinos to be considered as prompt

        tested = False
        topos = decomposer.decompose(
            model, sigmacut=0.1 * fb, doCompress=False, doInvisible=False, minmassgap=5.0 * GeV
        )
        toposExpected = {
            "[][1]": 1,
            "[][2]": 14,
        ...
        tested = False
        topos = decomposer.decompose(
            model, sigmacut=0.1 * fb, doCompress=False, doInvisible=True, minmassgap=5.0 * GeV
        )
        toposExpected = {
            "[][]": 1,
            "[][1]": 2,
        ...
        tested = False
        topos = decomposer.decompose(
            model, sigmacut=0.1 * fb, doCompress=True, doInvisible=False, minmassgap=5.0 * GeV
        )
        toposExpected = {
            "[][]": 1,
            "[][1]": 8,
        ...
        tested = False
        topos = decomposer.decompose(
            model, sigmacut=0.1 * fb, doCompress=True, doInvisible=True, minmassgap=5.0 * GeV
        )
        elIDs = {
            29 + 8: Element("[[[b]],[[b]]]", finalState=["MET", "MET"], model=finalStates),
