...
class IntegrationTest(unittest.TestCase):
    ...
    def predictions(self):
        return {"ATLAS-SUSY-2013-02": 572.168935 * fb, "CMS-SUS-13-012": 1.73810052766 * fb}

    def predchi2(self):
        return {
            "ATLAS-SUSY-2013-02": None,
            #            'CMS-SUS-13-012': 19.9647839329 } ## with 20% signal error
            "CMS-SUS-13-012": 41.42533308488771,
        }  ## thats with no signal error
    ...
    def testIntegration(self):
        ...
        smstoplist = decomposer.decompose(
            model, 0.1 * fb, doCompress=True, doInvisible=True, minmassgap=5.0 * GeV
        )
        listofanalyses = database.getExpResults(
            analysisIDs=["ATLAS-SUSY-2013-02", "CMS-SUS-13-012"], txnames=["T1"]
        )
    ...
    def checkPrediction(self, slhafile, expID, expectedValues, datasetID):
        ...
        self.configureLogger()
        smstoplist = decomposer.decompose(
            model, 0.0 * fb, doCompress=True, doInvisible=True, minmassgap=5.0 * GeV
        )
