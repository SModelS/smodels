...
if __name__ == "__main__":
    ...
    if File[-5:].lower()==".slha" or args.slha:
        if args.tofile:
            print "[xsecComputer.py] Computing cross section for slha file from %s, and adding to slha file." % File,
            for s in sqrtses:
              ss=addunit(s,'TeV')
            ...
            print "[xsecComputer.py] compute cross section for slha file, print out, but dont add to file. FIXME not yet implemented."
            sys.exit(0)
    if File[-4:].lower()==".lhe" or args.lhe:
        if args.tofile:
            print "[xsecComputer.py] compute cross section for lhe file, add to file. FIXME I guess we dont need this case?"
            sys.exit(0)
        else:
            print "[xsecComputer.py] compute cross section for lhe file, print out, but dont add to file. FIXME not yet implemented."
            sys.exit(0)
