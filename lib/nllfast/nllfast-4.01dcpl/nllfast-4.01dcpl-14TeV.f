c<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c        
c                         NLL-fast,  version 4.01dcpl @ 14 TeV
c
...
c are for LHC production cross sections at 14 TeV collision energy.
c
...
c... stops and squarks in the decoupling limit:
      ...
      IF ((PR.EQ.'st').OR.(PR.EQ.'sdcpl')) THEN  
         write(*,*)
         IF (PDF.EQ.'mstw') THEN
          write(*,*) '# LHC @ 14 TeV, MSTW2008NLO (MSTW2008LO for LO)'
         ELSEIF (PDF.EQ.'cteq') THEN
          write(*,*) '# LHC @ 14 TeV, CTEQ6.6M (CTEQ6L1 for LO)' 
         ENDIF 
         write(*,*) '# process: ', PR
     ...
     &---------------'
         ...
         write(23,*)
         IF (PDF.EQ.'mstw') THEN
          write(23,*) '# LHC @ 14 TeV, MSTW2008NLO (MSTW2008LO for LO)'
         ELSEIF (PDF.EQ.'cteq') THEN
          write(23,*) '# LHC @ 14 TeV, CTEQ6.6M (CTEQ6L1 for LO)' 
         ENDIF
         write(23,*) '# process: ', PR
...
c... open the grid file
       IF (PDF.EQ.'mstw') THEN  
        IF (PR.EQ.'st') THEN
            open(21,file="st_nllnlo_mstw2008_14TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'sdcpl') THEN
            open(21,file="sdcpl_nllnlo_mstw2008_14TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'gdcpl') THEN
            open(21,file="gdcpl_nllnlo_mstw2008_14TeV.grid",
     &           status='unknown')
         ELSE   
            write(*,*) 'WRONG INPUT PROCESS'
            STOP
        ...
        ELSEIF (PDF.EQ.'cteq') THEN
         IF (PR.EQ.'st') THEN
             open(21,file="st_nllnlo_cteq66_14TeV.grid",
     &            status='unknown')
         ELSEIF (PR.EQ.'sdcpl') THEN
            open(21,file="sdcpl_nllnlo_cteq66_14TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'gdcpl') THEN
            open(21,file="gdcpl_nllnlo_cteq66_14TeV.grid",
     &           status='unknown')
         ELSE
            write(*,*) 'WRONG INPUT PROCESS'
            STOP
