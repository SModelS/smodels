...
class Plotter(object):
    ...
    def loadParameters(self):
        ...
        try:
            spec = importlib.util.spec_from_file_location("parameters", self.parameterFile)
    ...
    def loadModelFile(self):
        ...
        if self.modelFile == None:
            ...
            try:
                spec = importlib.util.spec_from_file_location("BSMparticles", self.modelFile)
                parameters = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(parameters)
                self.particle_names = parameters
            except:
                logger.warning(
                    "Error loading model.py file %s , will use pdgs instead.", self.modelFile
                )
                self.particle_names = None
    ...
    def plot(self, outFolder, indexfile="plots.html"):
        ...
        logger.info(
            "Generation of interactive plots finished. Go to: \n %s/%s \n to see the plots."
            % (outFolder, indexfile)
        )
...
def main(args, indexfile="index.html"):
    ...
    if not os.path.exists(args.smodelsFolder):
        raise SModelSError(
            "directory of SModelS python output files: "
            + str(args.smodelsFolder)
            + "' does not exist"
        )
    ...
    if not loadData:
        raise SModelSError(
            "Error loading data from folders:\n %s\n %s" % (smodelsFolder, slhaFolder)
        )
