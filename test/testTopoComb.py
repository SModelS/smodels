...
class TopoCombTest(unittest.TestCase):
    def createSLHAFile(self, case="T1tttt"):
        ...
        ratios = {"T1": 0.0, "T5": 1.0}
        if case == "T1":
            ratios = {"T1": 1.0, "T5": 0.0}
        if case == "mixed":
            ratios = {"T1": 0.5, "T5": 0.5}
...
"""
    ...
    def testTopoComparison(self):
        ...
        el3 = Element(
            "[[[mu-,mu+],[mu-]],[[e-,e+],[mu-]]]", finalState=["HSCP", "MET"], model=finalStates
        )
        ...
        el3rev = Element(
            "[[[e-,e+],[mu-]],[[mu-,mu+],[mu-]]]", finalState=["MET", "HSCP"], model=finalStates
        )
        el4rev = Element(
            "[[[e-,e+],[mu-]],[[*,*]]]", finalState=["HSCP", "HSCP"], model=finalStates
        )
