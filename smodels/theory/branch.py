...
class Branch(object):
    ...
    def __init__(self, info=None, finalState=None, intermediateState=None, model=None):
        ...
        if isinstance(info, str):
            ...
            if not model or not hasattr(model, "getParticlesWith"):
                raise SModelSError(
                    "A Model object has to be defined when creating branches from strings."
                )
            else:
                branch = branch[0]
                ...
                for vertex in vertices:
                    ...
                    for pname in particleNames:
                        ...
                        if not smParticle:
                            raise SModelSError(
                                "Final state SM particle ``%s'' has not been defined in %s"
                                % (pname, model)
                            )
                        elif len(smParticle) != 1:
                            raise SModelSError(
                                "Ambiguous definition of label ``%s'' in %s"
                                % (smParticle[0].label, model)
                            )
                        else:
            ...
            if intermediateState:
                if not isinstance(intermediateState, list):
                    raise SModelSError(
                        "Intermediate state (``%s'') should be a list)" % intermediateState
                    )
            ...
            if len(bsmLabels) != self.vertnumb + 1:
                raise SModelSError(
                    "Number of intermediate states (``%s'') is not consistent)" % intermediateState
                )
            for bsmLabel in bsmLabels:
                bsmParticle = model.getParticlesWith(label=bsmLabel)
                if not bsmParticle:
                    raise SModelSError(
                        "BSM particle ``%s'' has not been defined in databaseParticles.py"
                        % bsmLabel
                    )
                elif len(bsmParticle) != 1:
                    raise SModelSError(
                        "Ambiguous definition of label ``%s'' in databaseParticles.py" % bsmLabel
                    )
                else:
    ...
    def __str__(self):
        ...
        sortedParticles = [
            sorted(vertex, key=lambda ptc: str(ptc)) for vertex in self.evenParticles
        ]
...
def decayBranches(branchList, sigcut=0.0):
    """
    ...
    while unstableBranches:
        ...
        for inbranch in unstableBranches:
            if sigcut > 0.0 and inbranch.maxWeight < sigcut:
...
class InclusiveBranch(Branch):
    ...
    def __init__(self, finalState=None, intermediateState=None, model=None):
        ...
        for bsmLabel in bsmLabels:
            ...
            if not bsmParticle:
                raise SModelSError(
                    "BSM particle ``%s'' has not been defined in model %s" % (bsmLabel, model)
                )
            elif len(bsmParticle) != 1:
                raise SModelSError(
                    "Ambiguous definition of label ``%s'' in model %s" % (bsmLabel, model)
                )
            else:
                self.oddParticles.append(bsmParticle[0])
