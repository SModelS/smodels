...
def getParticlesFromSLHA(slhafile):
    ...
    for b in qnumberBlocks:
        ...
        newParticle = Particle(
            ...
            label=label,
            pdg=pdg,
            eCharge=numbers[1] / 3.0,
            colordim=numbers[3],
            spin=(numbers[2] - 1.0) / 2.0,
        )
...
def load():
    ...
    # If failed, assume the input is an SLHA file:
        ...
        try:
            ...
            logger.error(
                "Could not load input model from %s. The file should be either a python module with particle definitions\
            or a SLHA file with QNUMBERS blocks."
                % modelFile
            )
            raise SModelSError()
    return BSMList
