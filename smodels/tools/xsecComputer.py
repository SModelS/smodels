...
class XSecComputer:
    ...
    def __init__(self, maxOrder, nevents, pythiaVersion, maycompile=True):
        ...
        if pythiaVersion not in [6, 8]:
            logger.error("Unknown pythia version %s. Allowed values: 6, 8" % (pythiaVersion))
            sys.exit()
    ...
    def addHigherOrders(self, sqrts, slhafile):
        ...
        while len(xsecs) > 0 and xsecs.getMinXsec() == 0.0 * pb:
            for xsec in xsecs:
                if xsec.value == 0.0 * pb:
                    xsecs.delete(xsec)
                    break
    ...
    def match(self, pids, theorypid):
        ...
        for spid in spids:
            if type(spid) == int:
                ...
                if type(spid) != str:
                    logger.error("I have a pid of type %s. Dont know what to do." % type(spid))
                    sys.exit()
    ...
    def applyMultipliers(self, xsecs, ssmultipliers):
        ...
        for pids in ssmultipliers.keys():
            if type(pids) not in [list, tuple]:
                logger.error(
                    "signal strength multipliers need to be supplied as a dictionary, with the keys being tuples of the mothers' pids, e.g. { (1000021, -1000001 ): 0.9, .... }"
                )
                sys.exit()
            if len(pids) != 2:
                logger.warning(
                    "currently we always only have two mothers, so why are the signal strength multipliers given for %d mothers?"
                    % len(pids)
                )
            known_pids = [3000006]  ## here we can define some exceptional pids
            for pid in pids:
                if type(pid) == int and (
                    abs(pid) < 1000000 or (abs(pid) > 3000000 and abs(pid) not in known_pids)
                ):
                    logger.warning(
                        "signal strength multiplier for pid %d supplied. what does that mean?" % pid
                    )
        for x in xsecs:
    ...
    def computeForOneFile(
        ...
        if tofile:
            logger.info("Computing SLHA cross section from %s, adding to " "SLHA file." % inputFile)
            ...
            for s in sqrtses:
                ...
                if tofile == "all":
                    xcomment = str(self.nevents) + " evts, pythia%d [pb]" % self.pythiaVersion
                    nXSecs += self.addXSecToFile(self.loXsecs, inputFile, xcomment, complain)
                    complain = False
                xcomment = str(self.nevents) + " events, [pb], pythia%d for LO" % self.pythiaVersion
                if tofile != False:
            ...
            for s in sqrtses:
                ss = s * TeV
                self.compute(
                    ss, inputFile, unlink=unlink, loFromSlha=lOfromSLHA, ssmultipliers=ssmultipliers
                )
    ...
    def computeForBunch(
        self, sqrtses, inputFiles, unlink, lOfromSLHA, tofile, pythiacard=None, ssmultipliers=None
    ):
        """compute xsecs for a bunch of slha files"""
        for inputFile in inputFiles:
            logger.debug("computing xsec for %s" % inputFile)
            self.computeForOneFile(
                sqrtses,
                inputFile,
                unlink,
                lOfromSLHA,
        ...
        for line in lines:
            if "Signal strength multipliers" in line:
                if line.strip() == newline:
                    ...
                    logger.error(
                        "Different signal strength multipliers have alread been applied!!!"
                    )
    ...
    def addXSecToFile(self, xsecs, slhafile, comment=None, complain=True):
        ...
        if xSectionList and complain:
            logger.info(
                "SLHA file already contains XSECTION blocks. Adding " "only missing cross sections."
            )
...
class ArgsStandardizer:
    ...
    def getSSMultipliers(self, multipliers):
        if type(multipliers) == str:
            ...
            if multipliers.count("{") != 1 or multipliers.count("}") != 1:
                logger.error(
                    "need to pass signal strengh multipliers as dictionary with tuple of pids as keys"
                )
    ...
    def checkAllowedSqrtses(self, order, sqrtses):
        ...
        for sqrts in sqrtses:
            if not sqrts in allowedsqrtses:
                logger.error(
                    "Cannot compute NLO or NLL xsecs for sqrts = %d "
    ...
    def writeToFile(self, args):
        ...
        if args.alltofile:
            if toFile == "highest":
                logger.warning("Specified both --tofile and --alltofile. Will use " "--alltofile")
            toFile = "all"
        return toFile
...
def main(args):
    ...
    for i in range(ncpus):
        ...
        if pid < 0:
            ...
            sys.exit()
        if pid == 0:
            logger.debug("chunk #%d: pid %d (parent %d)." % (i, os.getpid(), os.getppid()))
            logger.debug(" `-> %s" % " ".join(chunk))
            computer = XSecComputer(order, args.nevents, pythiaVersion, not args.noautocompile)
            toFile = canonizer.writeToFile(args)
            computer.computeForBunch(
                sqrtses,
                chunk,
                not args.keep,
                args.LOfromSLHA,
