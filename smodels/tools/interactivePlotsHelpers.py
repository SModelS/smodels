...
try:
    ...
    logger.error(
        "could not import plotly. Please try to install it, e.g. via:\npip install --user plotly"
    )
    sys.exit()
try:
    import pandas as pd
    ...
    logger.error(
        "could not import pandas. Please try to install it, e.g. via:\npip install --user pandas"
    )
    sys.exit()
import os
import pyslha
...
def importPythonOutput(smodelsFile):
    ...
    try:
        spec = importlib.util.spec_from_file_location("smodelsOutput", smodelsFile)
...
class Filler:
        ...
        if decompStatus == 1:
            ...
            for expres in expResList:
                if "r" in expres:
                    ...
                    r = getEntry(expres, "theory prediction (fb)") / getEntry(
                        expres, "upper limit (fb)"
                    )
                    r = Filler.truncate(self, r)
                if r > rmax:
                    rmax = r
    ...
    def getSlhaHoverInfo(self):
        ...
        for key in self.slha_hover_information.keys():
            ...
            if block == "MASS":
                self.data_dict.get(key).append(
                    Filler.truncate(self, abs(self.slhaData.blocks[block][code_number]))
                )
            else:
                self.data_dict.get(key).append(
                    Filler.truncate(self, self.slhaData.blocks[block][code_number])
                )
    ...
    def getCtau(self):
        ...
        for key in self.ctau_hover_information.keys():
            ...
            total_width = float(
                str(self.slhaData.decays[value]).split("total width = ")[1].split(" GeV")[0]
            )
            if total_width == 0:
                ctau = float("inf")
            else:
    ...
    def openSMParticles(self):
        ...
        spec = importlib.util.spec_from_file_location("sm_particles", sm_particle_file)
...
class PlotlyBackend:
    ...
    def makeDataFrame(self):
        ...
        self.data_frame_all.to_csv(
            self.path_to_plots + "/data_frame.txt", sep=" ", index=False, header=True
        )
    ...
    def fillHover(self):
        ...
        for column in self.slha_hover_information:
            if self.slha_hover_information.get(column)[0] == "MASS":
                self.data_frame_all["hover_text"] = (
                    ...
                    + column
                    + ": "
                    + self.data_frame_all[column].astype("str")
                    + " GeV"
                    + "<br>"
                )
            else:
        ...
        for column in self.BR_hover_information:
            ...
            self.data_frame_all["hover_text"] = (
                ...
                + column
                + ": "
                + data_frame_br[column].astype("str")
                + "<br>"
            )
        ...
        data_frame_provisional.loc[
            data_frame_provisional["SModelS_status"] == "False", "SModelS_status"
        ] = "Not-available"
        self.data_frame_all["hover_text"] = (
            self.data_frame_all["hover_text"]
            + self.html_names.get("SModelS_status")
            + ": "
            + data_frame_provisional["SModelS_status"]
            + "<br>"
        )
        ...
        if "r_max" in self.SModelS_hover_information:
            ...
            data_frame_provisional.loc[
                data_frame_provisional["r_max"] == "False", "r_max"
            ] = "Not-available"

            self.data_frame_all["hover_text"] = (
                self.data_frame_all["hover_text"]
                + self.html_names.get("r_max")
                + ": "
                + data_frame_provisional["r_max"]
                + "<br>"
            )
        ...
        if "Tx" in self.SModelS_hover_information:
            ...
            data_frame_provisional.loc[
                data_frame_provisional["Tx"] == "False", "Tx"
            ] = "Not-available"

            self.data_frame_all["hover_text"] = (
                self.data_frame_all["hover_text"]
                + self.html_names.get("Tx")
                + ": "
                + data_frame_provisional["Tx"]
                + "<br>"
            )
        if "Analysis" in self.SModelS_hover_information:

            data_frame_provisional.loc[
                data_frame_provisional["Analysis"] == "False", "Analysis"
            ] = "Not-available"

            self.data_frame_all["hover_text"] = (
                self.data_frame_all["hover_text"]
                + self.html_names.get("Analysis")
                + ": "
                + data_frame_provisional["Analysis"]
                + "<br>"
            )
        ...
        if "MT_max" in self.SModelS_hover_information:
            ...
            data_frame_provisional.loc[
                data_frame_provisional["MT_max"] == "False", "MT_max"
            ] = "Not-available"

            self.data_frame_all["hover_text"] = (
                self.data_frame_all["hover_text"]
                + self.html_names.get("MT_max")
                + ": "
                + data_frame_provisional["MT_max"]
                + "<br>"
            )
        if "MT_max_xsec" in self.SModelS_hover_information:

            data_frame_provisional.loc[
                data_frame_provisional["MT_max_xsec"] == "False", "MT_max_xsec"
            ] = "Not-available"

            self.data_frame_all["hover_text"] = (
                self.data_frame_all["hover_text"]
                + self.html_names.get("MT_max_xsec")
                + ": "
                + data_frame_provisional["MT_max_xsec"]
                + " [fb]"
                + "<br>"
            )
        if "MT_total_xsec" in self.SModelS_hover_information:
            ...
            data_frame_provisional.loc[
                data_frame_provisional["MT_total_xsec"] == "False", "MT_total_xsec"
            ] = "Not-available"

            self.data_frame_all["hover_text"] = (
                self.data_frame_all["hover_text"]
                + self.html_names.get("MT_total_xsec")
                + ": "
                + data_frame_provisional["MT_total_xsec"]
                + " [fb]"
                + "<br>"
            )
        ...
        if "MT_prompt_xsec" in self.SModelS_hover_information:
            ...
            data_frame_provisional.loc[
                data_frame_provisional["MT_prompt_xsec"] == "False", "MT_prompt_xsec"
            ] = "Not-available"

            self.data_frame_all["hover_text"] = (
                self.data_frame_all["hover_text"]
                + self.html_names.get("MT_prompt_xsec")
                + ": "
                + data_frame_provisional["MT_prompt_xsec"]
                + " [fb]"
                + "<br>"
            )
        ...
        if "MT_displaced_xsec" in self.SModelS_hover_information:
            ...
            data_frame_provisional.loc[
                data_frame_provisional["MT_displaced_xsec"] == "False", "MT_displaced_xsec"
            ] = "Not-available"

            self.data_frame_all["hover_text"] = (
                self.data_frame_all["hover_text"]
                + self.html_names.get("MT_displaced_xsec")
                + ": "
                + data_frame_provisional["MT_displaced_xsec"]
                + " [fb]"
                + "<br>"
            )
        ...
        if "MT_outgrid_xsec" in self.SModelS_hover_information:
            ...
            data_frame_provisional.loc[
                data_frame_provisional["MT_outgrid_xsec"] == "False", "MT_outgrid_xsec"
            ] = "Not-available"

            self.data_frame_all["hover_text"] = (
                self.data_frame_all["hover_text"]
                + self.html_names.get("MT_outgrid_xsec")
                + ": "
                + data_frame_provisional["MT_outgrid_xsec"]
                + " [fb]"
                + "<br>"
            )
        if "file" in self.SModelS_hover_information:
            data_frame_provisional.loc[
                data_frame_provisional["file"] == "False", "file"
            ] = "Not-available"
            self.data_frame_all["hover_text"] = (
                self.data_frame_all["hover_text"]
                + "file"
                + ": "
                + data_frame_provisional["file"]
                + "<br>"
            )
        return self.data_frame_all
    ...
    def DataFrameExcludedNonexcluded(self):
        ...
        self.data_frame_excluded = self.data_frame_all.loc[
            self.data_frame_all["SModelS_status"] == "Excluded"
        ]
        self.data_frame_nonexcluded = self.data_frame_all.loc[
            self.data_frame_all["SModelS_status"] == "Non-excluded"
        ]
    ...
    def makeContinuousPlots(self, data_frame, data_selection):
        ...
        for cont_plot in self.cont_plots:
            ...
            if (
                cont_plot == "MT_max_xsec"
                or cont_plot == "MT_total_xsec"
                or cont_plot == "MT_prompt_xsec"
                or cont_plot == "MT_displaced_xsec"
                or cont_plot == "MT_outgrid_xsec"
            ):
            ...
            data = [
                go.Scatter(
                    ...
                    marker=dict(
                        ...
                        color=z,
                        colorbar=dict(title=cont_plot_legend),
                        colorscale="Jet",
                    ),
                )
            ]
            ...
            if (
                self.variable_x.get(self.x_axis)[0] == "MASS"
                and self.variable_y.get(self.y_axis)[0] == "MASS"
            ):
                layout = go.Layout(
                    hovermode="closest",
            ...
            plotly.offline.plot(
                fig,
                filename=self.path_to_plots + "/" + cont_plot + "_" + data_selection + ".html",
                auto_open=False,
            )
        return
    ...
    def makeDiscretePlots(self, data_frame, data_selection):
        ...
        for disc_plot in self.disc_plots:
            ...
            fig = {
                "data": [
                    {
                        "x": data_frame_noFalse.loc[data_frame_noFalse[disc_plot] == value][
                            self.x_axis
                        ],
                        "y": data_frame_noFalse.loc[data_frame_noFalse[disc_plot] == value][
                            self.y_axis
                        ],
                        "name": value,
                        "mode": "markers",
                        "marker": dict(size=10),
                        "text": data_frame_noFalse.loc[data_frame_noFalse[disc_plot] == value][
                            "hover_text"
                        ],
                        "hoverinfo": "text",
                    }
            ...
            plotly.offline.plot(
                fig,
                filename=self.path_to_plots + "/" + disc_plot + "_" + data_selection + ".html",
                auto_open=False,
            )
        return
    ...
    def createIndexHtml(self):
        ...
        main_file.write(
            "<html><head><font size=6>SModelS interactive plots: "
            + self.plot_title
            + "</font></head>"
        )
        ...
        for plot in self.plot_list:
            ...
            for option in self.plot_data:
                ...
                plot_link = hyperlink_format.format(
                    link=plot_name + "_" + option + ".html", text=option
                )
    ...
    def makePlots(self, indexfile):
        ...
        data_frame_excluded, data_frame_nonexcluded = PlotlyBackend.DataFrameExcludedNonexcluded(
            self
        )
        ...
        if "non-excluded" in self.plot_data:
            ...
            if self.data_frame_nonexcluded.shape[0] == 0:
                logger.warning(
                    "Empty data frame for dataselection:non-excluded. Skipping this plots"
                )
                new_plot_data = []
                for option in self.plot_data:
