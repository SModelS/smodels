...
def addPromptAndDisplaced(branch,BR):    
    ...
    # keeping track of prompt and displaced decays within one branch, would make inclusive searches possible to include
    # but also number of possible combinations explodes
    """
    if len(F) > 1: 
        Plist = [list(P) for P in itertools.product(*F)]
        labelList = [list(l) for l in itertools.product(*labels)]
        F = []        
        for P in Plist:
            value = P[0]
