...
* *options*: main options for turning SModelS features on and off
  ...
  * **computeStatistics** (True/False): turns the likelihood and chi2 computation on and off 
    (see :ref:`likelihood calculation <likelihoodCalc>`). If True, the likelihood and chi2 values are computed for the most sensitive |EMr|.    
...
The stdout (or :ref:`log output <logOut>`) are intended to provide extensive information on the SModelS results.
...
* a list of all the |theory predictions| obtained and the corresponding |expres| upper limit.
  ...
  theory cross-section, the experimental observed upper limit, the (theory cross-section)/(observed upper limit) ratio (*r*) and 
  the (theory cross-section)/(expected upper limit) ratio (*r_expect*)
  the |ssigBRe|/(upper limit) 
  the BSM masses  the list of :ref:`condition values <ULconditions>`
  (if applicable), the |theory prediction| value (|ssigBRe|) and the corresponding experimental upper limit
  (see :ref:`confronting predictions <confrontPredictions>`) are also printed. 
  For |EMrs|, the corresponding signal region (|dataset|), the expected upper limit, the chi2 and likelihood values
  (if **computeStatistics** = True) are also shown.
...
* a list of all the |theory predictions| obtained and the corresponding |expres| upper limit.
  ...
  Finally, if  **computeStatistics** = True, the chi2 and likelihood values (for |EMrs|) are printed:
...
* information about the model coverage (if **testCoverage** = True).
  This includes the total input model cross-section which is not tested
  or constrained by any of the |express| in the |database| (see :ref:`coverage tool <topCoverage>` for more details)
  
.. literalinclude:: /images/summaryoutput.txt
   :lines: 120-123
       

* information about the missing topologies with highest cross-sections: 

.. literalinclude:: /images/summaryoutput.txt
   :lines: 127-131
   
* information about the topologies which are outside the  |express| grid
  (see :ref:`coverage tool <topCoverage>` for more details):

.. literalinclude:: /images/summaryoutput.txt
   :lines: 140-143   
   
* information about  topologies which are outside the  |express| grid
  (see :ref:`coverage tool <topCoverage>` for more details):
