...
class Model(object):
    ...
    def __init__(self, BSMparticles, SMparticles, label=None):
        ...
        self.BSMparticles = self.allBSMparticles[
            :
        ]  # at initialization assume all particles will be used
        ...
        for pdg in allPDGs:
            ...
            if len(p) > 1:
                raise SModelSError(
                    "PDG = %s has been defined for multiple particles (%s). Check your model definitions."
                    % (pdg, p)
                )
    ...
    def getModelDataFrom(self, inputFile):
        ...
        # If fails assume it is an LHE file:
            ...
            logger.info(
                "Using LHE input. All unstable particles will be assumed to have prompt decays."
            )
            logger.info(
                "Using LHE input. All particles not appearing in the events will be removed from the model."
            )
        finally:
            sys.stderr = storeErr
    ...
    def filterCrossSections(self):
        ...
        modelPDGs = [
            particle.pdg for particle in self.BSMparticles if isinstance(particle.pdg, int)
        ]
        for xsec in self.xsections.xSections[:]:
            for pid in xsec.pid:
                if not pid in modelPDGs:
                    logger.debug(
                        "Cross-section for %s includes particles not belonging to model and will be ignored"
                        % str(xsec.pid)
                    )
                    self.xsections.delete(xsec)
                if not pid in oddPDGs:
                    logger.debug(
                        "Cross-section for %s includes even particles and will be ignored"
                        % str(xsec.pid)
                    )
                    self.xsections.delete(xsec)
    ...
    def setMasses(self, massDict, roundMasses):
        ...
        for particle in self.BSMparticles:
            ...
            if not hasattr(particle, "pdg") or not hasattr(particle, "Z2parity"):
                raise SModelSError(
                    "PDG and/or Z2-parity for particle %s has not been defined" % particle.label
                )
            ...
            if abs(pdg) in massDict.keys():
                ...
                raise SModelSError(
                    "No mass found for %i in input file %s." % (particle.pdg, self.inputFile)
                )
    ...
    def setDecays(self, decaysDict, promptWidth, stableWidth, erasePrompt):
        ...
        for particle in self.BSMparticles:
            ...
            if particle.totalwidth < stableWidth:
                particle.totalwidth = 0.0 * GeV  # Treat particle as stable
                logger.debug(
                    "Particle %s has width below the threshold and will be assumed as stable"
                    % particle.pdg
                )
                continue
            ...
            if particle.totalwidth > promptWidth:
                ...
                logger.debug(
                    "Particle %s has width above the threshold and will be assumed as prompt."
                    % particle.pdg
                )
                ...
                particle.decays.append(
                    None
                )  # Include possibility for particle being long-lived (non-prompt)
            ...
            for decay in particleData.decays:
                ...
                if missingIDs:
                    logger.info(
                        "Particle(s) %s is not defined within model. Decay %s will be ignored"
                        % (missingIDs, decay)
                    )
                    continue
                ...
                if len(oddPids) != 1 or len(evenPids + oddPids) != len(decay.ids):
                    logger.debug(
                        "Decay %i -> %s is not of the form Z2-odd -> Z2-odd + [Z2-even particles] and will be ignored"
                        % (pdg, pids)
                    )
                    continue
                ...
                decayIDs = [
                    pid * chargeConj if pid * chargeConj in allPDGs else pid for pid in decay.ids
                ]
                newDecay = pyslha.Decay(
                    br=decay.br, nda=decay.nda, parentid=decay.parentid, ids=decayIDs
                )
                ...
                for pdg in newDecay.ids:
                    ...
                    if not daughter:
                        raise SModelSError(
                            "Particle with PDG = %i was not found in model. Check the model definitions."
                            % pdg
                        )
                    elif len(daughter) > 1:
                        raise SModelSError(
                            "Multiple particles defined with PDG = %i. PDG ids must be unique."
                            % pdg
                        )
                    else:
                        daughter = daughter[0]
    ...
    def updateParticles(
        ...
        if len(self.BSMparticles) == len(self.allBSMparticles):
            ...
            logger.info(
                "Loaded %i BSM particles (%i particles not found in %s)"
                % (
                    len(self.BSMparticles),
                    len(self.allBSMparticles) - len(self.BSMparticles),
                    self.inputFile,
                )
            )
