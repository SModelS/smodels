...
def compareXSections(dictA, dictB, nevts, relError=0.1):
    ...
    for xsec in commonXsecs:
        ...
        if (
            ...
            print(
                "Cross-section for %s differ by (%s +- %s) fb"
                % (str(xsec), str(diff.asNumber(fb)), str(mcError.asNumber(fb)))
            )
    ...
    for xsec in missingXsecs:
        if xsec in dictA:
            if list(dictA[xsec].values())[0] > 2.0 * mcError:
                print("Cross-section for %s missing" % str(xsec))
                print("    %s" % (list(dictA[xsec].values())[0]))
                equalXsecs = False
        else:
            if list(dictB[xsec].values())[0] > 2.0 * mcError:
                print("Cross-section for %s missing" % str(xsec))
...
class XSecTest(unittest.TestCase):
    ...
    def testLOGlu(self):
        ...
        w8 = computer8.compute(
            8 * TeV, slhafile, pythiacard="./pythia8_to_pythia6.cfg"
        ).getDictionary()
