...
class ProxyDBCreater:
    ...
    def create(self, servername, serverport):
        ...
        self.database.client = DatabaseClient(
            servername, serverport, verbose=self.verbstring, rundir=self.rundir, clientid=self.nr
        )
        for e, expRes in enumerate(self.database.expResultList):
            for d, dataset in enumerate(expRes.datasets):
                for t, txn in enumerate(dataset.txnameList):
                    self.database.expResultList[e].datasets[d].txnameList[t].dbClient = copy.copy(
                        self.database.client
                    )
                    del self.database.expResultList[e].datasets[d].txnameList[t].txnameData.tri
                    if txn.txnameDataExp != None:
                        del (
                            self.database.expResultList[e]
                            .datasets[d]
                            .txnameList[t]
                            .txnameDataExp.tri
                        )
    ...
    def pprint(self, *args):
        if self.verbose > 25:
            print("[proxyDBCreater-%s] %s" % (time.strftime("%H:%M:%S"), " ".join(map(str, args))))
    ...
    def symlink(self):
        ...
        self.pprint("setting a symlink from %s to %s" % (self.outputfile, symfile))
    ...
    def run(self, really):
        ...
        if really:
            self.pprint("starting a server on %s: %s" % (self.servername, servercmd))
            import subprocess
