...
with open("orig/ANA-SUSY-2019-09-PAPER.csv", newline="") as csvfile:
    reader = csv.reader(
        csvfile, delimiter=" "
    )  # Every space is a seperator between two elements of the row
    for row in reader:  # We go rw by row
        if row != []:  # If the row is not empty
            ...
            if row[0] in ["{Regions}", "{Regions", "Region"]:  # If the row contains the SR names
                list_SR = (
                    []
                )  # We store the SR order in order to write the following yields in the corresponding dictionary
                for element in row:
                    if (
                        element != "" and "\SR" in element
                    ):  # If the element of the row really is a SR name
                        element = replace(
                            element
                        )  # We reshape the SR name to match the name in the corresponding .csv file title
                        topo = element[
                            0 : element.index("_")
                        ]  # The name 'topo' is used to easily loop over a given topology (not mandatory). We only keep the topology part of the name
                        list_SR.append(
                            element
                        )  # Store the SR in the list of SR names, at the following of the others
                        if topo not in tables:  # If it is the first time we encounter this topology
                            tables[topo] = {
                                element: {"observed": None, "expected": None, "bgError": None}
                            }
            ...
            if row[0] == "Observed":  # If the row corresponds to the 'observed' yields
                index_SR = (
                    0  # We want to write the yields in the order given by the previous 'Region' row
                )
                # row = clean_row(row)
                for element in row:
                    if "$" in element:  # If the element is a number
                        tables[topo][list_SR[index_SR]]["observed"] = float(
                            replace(element)
                        )  # Add the number to the dictionary following the order given by list_SR (the order of the SR obtained according to the previous 'Region' row)
            ...
            if row[0] == "Fitted":  # If the row corresponds to the 'expected' and 'bgError' yields
                ...
                for element in row:
                    if "$" in element:  # If the element is a number
                        if (
                            bgError
                        ):  # If the 'expected' yield has already been written, the next number is the corresponding 'bgError'
                            tables[topo][list_SR[index_SR]]["bgError"] = float(
                                replace(element)
                            )  # Same as for the 'Observed' row
                            ...
                            tables[topo][list_SR[index_SR]]["expected"] = float(
                                replace(element)
                            )  # Same as for the 'Observed' row
...
for topo in tables:  # To go through the results
    for SR in tables[topo]:
        ...
        for event in tables[topo][SR]:
            if (
                tables[topo][SR][event] == None
            ):  # Check that every entry of the dictionary has been rewritten
        ...
        if topo == "SRWZ":
            ...
            # +++++++ dataset block ++++++++++++++
            locals()[SR] = DataSetInput(
                SR
            )  # locals() converts its string argument inside [] into a local variable
            locals()[SR].setInfo(
                dataType="efficiencyMap",
                dataId=SR,
                observedN=tables[topo][SR]["observed"],
                expectedBG=tables[topo][SR]["expected"],
                bgError=tables[topo][SR]["bgError"],
            )
            ...
            TChiWZ_1.setSources(
                dataLabels=["obsExclusion", "expExclusion"],
                dataFiles=[
                    "orig/Fig16aWZExclusion:Wino-bino(+),onshell_Obs.csv",
            ...
            TChiWZ_1.addSource(
                "efficiencyMap", "orig/" + SR + "_efficiency.csv", unit="", dataFormat="csv"
            )
        ...
        if topo == "SRlow" or topo == "SRhigh":
            ...
            TChiWZoff_plus.setSources(
                dataLabels=["obsExclusion", "expExclusion"],
                dataFiles=[
                    "orig/Fig16bWZExclusion:Wino-bino(+)(Deltam),offshell_Obs.csv",
                    "orig/Fig16bWZExclusion:Wino-bino(+)(Deltam),offshell_Exp.csv",
                ],
                units=[None] * 2,
                dataFormats=["csv"] * 2,
            )
            TChiWZoff_plus.addSource(
                "efficiencyMap",
                "orig/" + SR + "_cuts_winobino(+)_efficiency.csv",
                unit="",
                dataFormat="csv",
            )
            ...
            TChiWZoff_hino.exclusionDataUrl = (
                "https://atlas.web.cern.ch/Atlas/GROUPS/PHYSICS/PAPERS/SUSY-2019-09/fig_16d.png"
            )
            TChiWZoff_hino.dataUrl = "https://doi.org/10.17182/hepdata.95751.v1/r3"
            TChiWZoff_hino.setSources(
                dataLabels=["obsExclusion", "expExclusion"],
                ############ Use global (offshell+compressed) or onshell only exclusion curves ? ##############
                dataFiles=[
                    "orig/Fig16dWZExclusion:Higgsino(Deltam),offshell_Obs.csv",
            ...
            TChiWZoff_hino.addSource(
                "efficiencyMap",
                "orig/" + SR + "_cuts_higgsino_efficiency.csv",
                unit="",
                dataFormat="csv",
            )
...
# +++++++ dataset block ++++++++++++++
SR_WH_0j = DataSetInput(
    "SR_WH_0j"
)  ####################################### No clear description of this more inclusive SR, need to count the SR with n_jet=0 and mll>105 GeV ?
####################################### Here the SR with mll>105 GeV were not taken into account, even when n_jet=0
SR_WH_0j.setInfo(
    dataType="efficiencyMap", dataId="SR_WH_0j", observedN=261, expectedBG=244, bgError=14
)
...
TChiWH_1.exclusionDataUrl = (
    "https://atlas.web.cern.ch/Atlas/GROUPS/PHYSICS/PAPERS/SUSY-2019-09/fig_17.png"
)
...
TChiWH_1.addSource(
    "efficiencyMap",
    (
        "orig/AuxFig11aAcc:OnshellSR_{low-m_{ll}-0j}^{Wh}.csv",
        "orig/AuxFig11bEff:OnshellSR_{low-m_{ll}-0j}^{Wh}.csv",
    ),
    unit="/100000",
    dataFormat="mcsv",
)
...
SR_WH_nj.setInfo(
    dataType="efficiencyMap", dataId="SR_WH_nj", observedN=488, expectedBG=492.6, bgError=26
)
...
TChiWH_1.addSource(
    "efficiencyMap",
    (
        "orig/AuxFig11cAcc:OnshellSR_{low-m_{ll}-nj}^{Wh}.csv",
        "orig/AuxFig11dEff:OnshellSR_{low-m_{ll}-nj}^{Wh}.csv",
    ),
    unit="/100000",
    dataFormat="mcsv",
)
...
SR_WH_DFOS.setInfo(
    dataType="efficiencyMap", dataId="SR_WH_DFOS", observedN=20, expectedBG=11.5, bgError=2.4
)
...
TChiWH_1.addSource(
    "efficiencyMap",
    ("orig/AuxFig11eAcc:OnshellSR_{DFOS}^{Wh}.csv", "orig/AuxFig11fEff:OnshellSR_{DFOS}^{Wh}.csv"),
    unit="/100000",
    dataFormat="mcsv",
)
