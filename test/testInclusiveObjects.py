...
class InclusiveObjectsTest(unittest.TestCase):
    ...
    def testInclusiveVertex(self):
        ...
        z = branch.InclusiveBranch(
            finalState="MET", intermediateState=["gluino", "C1+"], model=finalStates
        )
        self.assertTrue(x == z)

        z = branch.InclusiveBranch(finalState="MET", intermediateState=["C1+"], model=finalStates)
        self.assertFalse(x == z)
        z = branch.InclusiveBranch(
            finalState="MET", intermediateState=["C1+", "gluino"], model=finalStates
        )
        self.assertFalse(x == z)
    ...
    def testInclusiveElement(self):
        ...
        el1 = element.Element(
            info="[[[e+],[e-,mu+]],[[*],[e-,mu+]]]", finalState=["MET", "HSCP"], model=finalStates
        )
        el2 = element.Element(
            info="[[[e+],[e-,mu+]],[[jet],[e-,mu+]]]",
            finalState=["anyOdd", "HSCP"],
            model=finalStates,
        )
        el3 = element.Element(
            info="[[[e+],[e-,mu+]],[[jet],[e-,mu+]]]",
        ...
        el4 = element.Element(
            info="[[*],[[jet],[e-,mu+]]]", finalState=["MET", "HSCP"], model=finalStates
        )
        el5 = element.Element(
            info="[[[e+],[e-,mu+]],[[jet],[e-,mu+]]]", finalState=["MET", "HSCP"], model=finalStates
        )
        el6 = element.Element(info="[[*],[*]]", finalState=["MET", "HSCP"], model=finalStates)
        el7 = element.Element(
            info="[[[e+],[e-,mu+]],[[jet,jet],[e-,mu+]]]",
            finalState=["MET", "HSCP"],
            model=finalStates,
        )
        ...
        el13 = element.Element(
            info="[[[q,q],[e-,nu]],[]]",
            finalState=["MET", "MET"],
            intermediateState=[["gluino", "C1+"], []],
            model=finalStates,
        )
        el14 = element.Element(
            info="[['*'],['*']]",
            finalState=["MET", "MET"],
            intermediateState=[["gluino", "C1+"], ["C1+"]],
            model=finalStates,
    ...
    def testInclusiveTxNameData(self):
        ...
        res = tx.txnameData.getValueFor([[100.0 * GeV]] * 2)
        self.assertAlmostEqual(res, 0.058038)
        res = tx.txnameData.getValueFor([[500.0 * GeV, 150.0 * GeV, 10.0 * GeV], [100.0 * GeV]])
        self.assertAlmostEqual(res, 0.058038)

        res = tx.txnameData.getValueFor([[125.0 * GeV]] * 2)
        self.assertAlmostEqual(res, 0.090999)
        res = tx.txnameData.getValueFor([[200.0 * GeV]] * 2)
        ...
        gInfo.addInfo("dataId", "c000")
        tx = TxName(f, gInfo, gInfo, finalStates)
        res = tx.txnameData.getValueFor([[279.0 * GeV, 170.0 * GeV, 100.0 * GeV]] * 2)
        self.assertAlmostEqual(res, 0.097172, 6)
        res = tx.txnameData.getValueFor([[100.0 * GeV], [279.0 * GeV, 170.0 * GeV, 100.0 * GeV]])
        self.assertAlmostEqual(res, 0.097172, 6)
        res = tx.txnameData.getValueFor(
            [[500 * GeV, 100.0 * GeV], [1.917e03 * GeV, 1.7e02 * GeV, 1e02 * GeV]]
        )
        self.assertAlmostEqual(res, 0.00025745, 6)

        res = tx.txnameData.getValueFor(
            [[500 * GeV, 100.0 * GeV, 10.0 * GeV], [1.112e03 * GeV, 188 * GeV, 1e02 * GeV]]
        )
