...
class ReweightingTest(unittest.TestCase):
    def testcalculateProbabilities(self):
        ...
        prob = calculateProbabilities(
            gluino.totalwidth.asNumber(GeV), Leff_inner=0.000769, Leff_outer=7.0
        )
        F_long, F_prompt, F_displaced = prob["F_long"], prob["F_prompt"], prob["F_displaced"]
        self.assertAlmostEqual(F_long, 1.0)
        self.assertEqual(F_prompt, 0.0)
        self.assertAlmostEqual(F_displaced, 0.0)
    ...
    def testreweightFactorFor(self):
        ...
        self.assertAlmostEqual(f, 0.0, places=3)
