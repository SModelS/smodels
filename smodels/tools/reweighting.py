...
def defaultEffReweight(element, Leff_inner=None, Leff_outer=None, minWeight=1e-10):
    """
    ...
    for branchWidths in elWidths:
        ...
        branchFraction *= calculateProbabilities(branchWidths[-1], Leff_inner, Leff_outer)["F_long"]
...
def defaultULReweight(element, Leff_inner=None, Leff_outer=None):
    ...
    if not effFactor:
        ...
        return 1.0 / effFactor


def reweightFactorFor(element, resType="prompt", Leff_inner=None, Leff_outer=None):
    """
    ...
    FpromptTotal = 1.0  # Keep track of the probability of all decays being prompt
    FlongTotal = 1.0  # Keep track of the probability for at least one decay being long-lived
    for branch in element.branches:
        for particle in branch.oddParticles[:-1]:
            ...
            if width == float("inf") * GeV:
                ...
                probs = calculateProbabilities(width.asNumber(GeV), Leff_inner, Leff_outer)
    ...
    for branch in element.branches:
        ...
        if width == float("inf") * GeV:
            ...
            Flong = calculateProbabilities(width.asNumber(GeV), Leff_inner, Leff_outer)["F_long"]
        FdispTotal *= Flong
        FpromptTotal *= Flong
...
def calculateProbabilities(width, Leff_inner, Leff_outer):
    ...
    F_prompt = 1.0 - exp(-width * Leff_inner / hc)
    F_displaced = 1.0 - F_prompt - F_long
