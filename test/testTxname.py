...
class TxTest(unittest.TestCase):
    ...
    def testCoordinateTransf(self):
        ...
        initial = [[300.0 * GeV, 100.0 * GeV], [300.0 * GeV, 100.0 * GeV]]
        coords = txname.txnameData.dataToCoordinates(
            initial, txname.txnameData._V, txname.txnameData.delta_x
        )
        data = txname.txnameData.coordinatesToData(
            coords, txname.txnameData._V, txname.txnameData.delta_x
        )
        data = np.array(data)
        ...
        expRes = database.getExpResults(
            analysisIDs=["ATLAS-SUSY-2016-08"], datasetIDs=[None], txnames=["T5Disp"]
        )
        txname = expRes[0].datasets[0].txnameList[0]
        initial = [
            [(300.0 * GeV, 1e-16 * GeV), 100.0 * GeV],
            [(300.0 * GeV, 1e-16 * GeV), 100.0 * GeV],
        ]
        coords = txname.txnameData.dataToCoordinates(
            initial, txname.txnameData._V, txname.txnameData.delta_x
    ...
    def testCoordinateTransfInclusive(self):
        ...
        expRes = database.getExpResults(
            analysisIDs=["CMS-EXO-13-006"], datasetIDs=["c000"], txnames=["THSCPM4"]
        )
        txname = expRes[0].datasets[0].txnameList[0]
        initial = [
            [(300.0 * GeV, 1e-16 * GeV), 100.0 * GeV],
        ...
        data = np.array(data, dtype=object)
        newInitial = ["*", [300.0 * GeV, 100.0 * GeV]]
        ...
        initialFlat = np.array(
            [x.asNumber(GeV) if str(x) != "*" else -1 for x in flattenArray(newInitial)]
        )
        diff = np.linalg.norm(dataFlat - initialFlat)
        self.assertAlmostEqual(diff, 0.0)
        ...
        expRes = database.getExpResults(
            analysisIDs=["CMS-EXO-13-006"], datasetIDs=["c000"], txnames=["THSCPM7"]
        )
        txname = expRes[0].datasets[0].txnameList[0]
        initial = [
            [(300.0 * GeV, 1e-16 * GeV), 50.0 * GeV],
            [(200.0 * GeV, 1e-18 * GeV), 100.0 * GeV, 1.0 * GeV],
        ]
        coords = txname.txnameData.dataToCoordinates(
            initial, txname.txnameData._V, txname.txnameData.delta_x
        )
        ...
        data = np.array(data, dtype=object)
        newInitial = [[300.0 * GeV, 50.0 * GeV], [200.0 * GeV, 100.0 * GeV, 1.0 * GeV]]
        newInitial = np.array(newInitial, dtype=object)
        self.assertEqual(data.shape, newInitial.shape)
        dataFlat = np.array([x.asNumber(GeV) for x in flattenArray(data)])
