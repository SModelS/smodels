...
class OutputStatus(object):
    ...
    def __init__(self, status, inputFile, parameters, databaseVersion):
        ...
        try:
            filename = os.path.join(installation.installDirectory(), "smodels/version")
...
class SlhaStatus(object):
    ...
    def __init__(
        self, filename, findMissingDecayBlocks=True, findIllegalDecays=False, checkXsec=True
    ):
    ...
    def read(self):
        ...
        """
        try:
            ret = pyslha.readSLHAFile(self.filename)
        except (pyslha.ParseError, IOError):
            return None
        if not ret.blocks["MASS"]:
            return None
        return ret
    ...
    def evaluateStatus(self):
        ...
        (
            ...
            self.decayBlocksStatus
        )  # add only warning, no negative staus flag in case of missing decay blocks
        if st < 0:
            warning = True
            retMes += message + "\n"
    ...
    def emptyDecay(self, pid):
        ...
        """
        if not abs(pid) in self.slha.decays:
            return True  # consider missing decay block as empty
        if not self.slha.decays[abs(pid)].decays:
            return True
        return None
    ...
    def findMissingDecayBlocks(self, findMissingBlocks):
        ...
        if st == 1:
            ...
            msg = "# Missing decay blocks for %s" % str(missing)
        return st, msg
    ...
    def findIllegalDecay(self, findIllegal):
        ...
        for particle, block in self.slha.decays.items():
            if particle in SMpdgs:
                continue
            if not particle in self.slha.blocks["MASS"].keys():
                continue
                ...
                for ptc in dcy.ids:
                    ...
                    if ptc in SMpdgs:
                        ...
                        if not smParticle:
                            ...
                            raise SModelSError(
                                "Multiple particles defined with PDG = %i in model" % ptc
                            )
                        else:
                        ...
                        return -2, "Unknown PID %s in decay of %s" % (
                            str(ptc),
                            str(particle) + ". Add " + str(ptc) + " to smodels/particle.py",
                        )
                if mDau > mMom:
                    st = -1
