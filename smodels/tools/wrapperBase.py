...
class WrapperBase(object):
    ...
    def checkInstallation(self, compile=True):
        ...
        if not os.path.exists(self.executablePath):
            if self.maycompile:  ## should have worked
                logger.error(
                    "Compilation of %s failed. Is a according compiler installed?" % self.name
                )
            self.complain()
    ...
    def tempDirectory(self):
        ...
        if self.tempdir in [None, ""]:
            self.tempdir = tempfile.mkdtemp(prefix="xsec", dir=self.defaulttempdir)
    ...
    def complain(self):
        logger.error(
            "please fix manually, e.g. try 'make' in smodels/lib, "
