...
class GenericSMS(object):
    ...
    def __init__(self):
        ...
        self._genIndexIteratorCache = {} # Cache for genIndexIterator results
    ...
    def compressToFinalStates(self,compressPrimary=False):
        ...
        :param compressPrimary: If True, it will also remove the primary mothers
        """
        ...
        for m in mothers:
            ...
            if self.out_degree(m) == 0:
                continue
            # Remove all nodes for subtree
            # and collect the final state nodes
            fsNodes = []
            allDaughters = list(self.dfsIndexIterator(m))
            fsNodes = [self.indexToNode(n) for n in allDaughters
            ...
            smsComp.add_edges_from(product([m],nIndices))
