...
class Tree(object):
    ...
    def getFinalStates(self, source=None):
        ...
        finalStates = []
        for mom, daughters in self.bfs_successors(source.node):
            final = [d for d in daughters if not self.out_degree(d)]
            finalStates += [self.nodesMapping[d].particle for d in final]
    ...
    def getSubTree(self, nodeIndex):
        ...
        subTree = Tree()
        for mom, daughters in self.bfs_successors(nodeIndex,
                                                  includeLeaves=True):

            subTree.nodesMapping[mom] = self.nodesMapping[mom]
            subTree.successors[mom] = daughters[:]
            for d in daughters:
                subTree.predecessors[d] = mom

        subTree._root = subTree.nodesMapping[nodeIndex]
        return subTree
    ...
    def sort(self):
        ...
        subTrees = []
        for d in self.successors[self.root.node][:]:
            subTree = self.getSubTree(d)
            subTree.sort()
