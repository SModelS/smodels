...
.. module:: SMSDataObjects
   :synopsis: Our basic data objects: TopologyList, GTop, EElement, Branch
...
class Branch:
  ...
  def describe ( self ):
    """ a lengthy description of the Branch """
      ...
      for ckey in conds_num.keys():
        ...
        for weight in zeroweight.keys():
          ...
          try:                                           #Try to convert from unitless Unum to number
            if type(res[weight]) == type(addunit(1,'fb')) and res[weight] == res[weight].asNumber():
              res[weight] = res[weight].asNumber()
          except:
            pass
        cond_res[ckey] = res   
