...
class Browser(object):
    ...
    def getEfficiencyFor(self, expid, dataset, txname, massarray):
        ...
        if not expres:
            logger.warning(
                "Could not find efficiencyMap result %s."
                " getEfficiencyForr can only be\
            used for efficiencyMap results."
                % (expid)
            )
            return None
    ...
    def getULFor(self, expid, txname, massarray, expected=False):
        ...
        if not expres:
            logger.warning(
                "Could not find UL result %s. getULFor can only be\
            used for upper-limit results."
                % (expid)
            )
            return None
    ...
    def getULForSR(self, expid, datasetID):
        ...
        if not expres:
            logger.warning(
                "Could not find efficiency-map result %s . getULForSR can only be\
            used for efficiency-map results."
                % (expid)
            )
            return None
    ...
    def selectExpResultsWith(self, **restrDict):
        ...
        for tag, value in restrDict.items():
            if tag in allfields:
                ...
                logger.warning("Field/attribute %s not found (will be ignored)." % tag)
        ...
        if not self._selectedExpResults:
...
def main(args):
    ...
    def getHeader():
        ...
        header += (
            "More examples on how to access the database can be found in the SModelS manual.\n"
        )
