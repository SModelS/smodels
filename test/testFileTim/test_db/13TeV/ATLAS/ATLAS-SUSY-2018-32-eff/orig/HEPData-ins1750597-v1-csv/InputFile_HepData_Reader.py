...
'''
SR	= ['SR-DF-0J','SR-DF-1J','SR-SF-0J','SR-SF-1J']
MT2 = ['100,inf','160,inf','100,120','120,160']

for sr in SR:
    for mt2 in MT2:
        acc = 'Acceptance' + sr + '-[' + mt2 + ')forC1C1WWgrid.csv'
        eff = 'Efficiency' + sr + '-[' + mt2 + ')forC1C1WWgrid.csv'

        Map_Multiplier('TChiWW',sr+'_mt2=[' + mt2 + ')',acc,eff,2,2,1) #norm=10000
'''
...
for sr in SR:
    for mt2 in MT2:
        ...
        accFile = 'Acceptance' + sr + '-[' + mt2 + ')forC1C1WWgrid.csv'
        effFile = 'Efficiency' + sr + '-[' + mt2 + ')forC1C1WWgrid.csv'
        #accSF = 'Acceptance' + 'SR-SF-' + sr + '-[' + mt2 + ')forC1C1WWgrid.csv'
        #effSF = 'Efficiency' + 'SR-SF-' + sr + '-[' + mt2 + ')forC1C1WWgrid.csv'

        fileName = 'EffMap_TChiWW_' + sr + '_mt2=[' + mt2 + ')' + '.txt'

        X1,Y1,ACC = Reading_Values(accFile,2,2)
        X2,Y2,EFF = Reading_Values(effFile,2,2)
        #X1_SF,Y1_SF,Acc_SF = Reading_Values(accSF,2,2)
        #X2_SF,Y2_SF,Eff_SF = Reading_Values(effSF,2,2)


        outputMap = open(fileName,'w')
        outputMap.write('# MassX , MassY , Eff*Acc  '+'\n')

        for i in range(len(ACC)):
            #if X1_DF[i]==X2_DF[i] and Y1_DF[i]==Y2_DF[i]:

                #df = float(Acc_DF[i]) * float(Eff_DF[i]) * 1E-4
                #sf = float(Acc_SF[i]) * float(Eff_SF[i]) * 1E-4

            outputMap.write(X1[i] + ' ' + Y1[i] + ' ' + str(float(ACC[i])*float(EFF[i])*1E-4) + '\n')

        outputMap.close()			
        print "Map ",fileName, ' written!'
