c<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
c        
c            NLL-fast,  version 5.01 (for decoupled cases and stops) @ 33 TeV
c
...
c are for LHC production cross sections at 33 TeV collision energy.
c
...
c
c After compiling the code, it should be called in the following way:
c
c for the case of stop-antistop (sbottom-antisbottom) production:
c
...
c -- For stop-pair production, SUSY parameters other than stop masses correspond to CMSSM benchmark
c    point 40.2.5 as defined in arXiv:1109.3859. For stop masses lower than gluino mass and with
c    the gluino mass lower than the one in the benchmark point 40.2.5, results differ from results
c    given by NLL-fast by less than 1%. Other choices of SUSY parameters will lead to bigger
c    differences with NLL-fast results (however, by not more than 10%). In particular, in the case
c    of stop-production in the limit of all other sparticles decoupled, the results of NLL-fast are
c    accurate within 1% for stop masses up to ~1.5 TeV. Please contact us if you need precise results
c    for other choices of SUSY parameters. 
...
c>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
     ...
     &     NLLMI,DNLL,DNLLPL,DNLLMI
      ...
      REAL*8 Y1(200,200),Y2(200,200),Y12(200,200),G1,G2,D1(4),D2(4),
     &     D12(4)
      CHARACTER*5 CR,PR

      DIMENSION MSQGR(200),MGLGR(200),LOGR(200,200),LOGRPL(200,200),
     &     LOGRMI(200,200),NLOGR(200,200),NLOGRPL(200,200),
     &     NLOGRMI(200,200),PDFGRPL(200,200),PDFGRMI(200,200),
     &     ASGRPL(200,200),ASGRMI(200,200),LLOGR(200,200),
     &     LLOGRPL(200,200),LLOGRMI(200,200),LNLOGR(200,200),
     &     LNLOGRPL(200,200),LNLOGRMI(200,200),NLLGR(200,200),
     &     NLLGRPL(200,200), NLLGRMI(200,200),LNLLGR(200,200),
     &     LNLLGRPL(200,200),LNLLGRMI(200,200)  
...
c... for this version disable option tot
      IF ((PR.NE.'gdcpl').AND.(PR.NE.'sdcpl').
     &    AND.(PR.NE.'st')) THEN  
...
c... stops and squarks in the decoupling limit:
      ...
      IF ((PR.EQ.'st').OR.(PR.EQ.'sdcpl')) THEN  
         write(*,*)
         IF (PDF.EQ.'mstw') THEN
          write(*,*) '# LHC @ 33 TeV, MSTW2008NLO (MSTW2008LO for LO)'
         ELSEIF (PDF.EQ.'cteq') THEN
          write(*,*) '# LHC @ 33 TeV, CTEQ6.6M (CTEQ6L1 for LO)' 
         ENDIF 
         write(*,*) '# process: ', PR
     ...
     &---------------'
         ...
         write(23,*)
         IF (PDF.EQ.'mstw') THEN
          write(23,*) '# LHC @ 33 TeV, MSTW2008NLO (MSTW2008LO for LO)'
         ELSEIF (PDF.EQ.'cteq') THEN
          write(23,*) '# LHC @ 33 TeV, CTEQ6.6M (CTEQ6L1 for LO)' 
         ENDIF
         write(23,*) '# process: ', PR
...
c... open the grid file
       IF (PDF.EQ.'mstw') THEN  
        IF (PR.EQ.'st') THEN
            open(21,file="st_nllnlo_mstw2008_33TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'sdcpl') THEN
            open(21,file="sdcpl_nllnlo_mstw2008_33TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'gdcpl') THEN
            open(21,file="gdcpl_nllnlo_mstw2008_33TeV.grid",
     &           status='unknown')
         ELSE   
            write(*,*) 'WRONG INPUT PROCESS'
            STOP
        ...
        ELSEIF (PDF.EQ.'cteq') THEN
         IF (PR.EQ.'st') THEN
             open(21,file="st_nllnlo_cteq66_33TeV.grid",
     &            status='unknown')
         ELSEIF (PR.EQ.'sdcpl') THEN
            open(21,file="sdcpl_nllnlo_cteq66_33TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'gdcpl') THEN
            open(21,file="gdcpl_nllnlo_cteq66_33TeV.grid",
     &           status='unknown')
         ELSE
            write(*,*) 'WRONG INPUT PROCESS'
            STOP
...
c.......................................................................
     ...
     &       MSQ,MGL,INTPOUT,DINTPOUT,DATAINGRST
      DIMENSION MSQGR(200),MGLGR(200),DATAGR(200,200),DATAINGRST(200)
