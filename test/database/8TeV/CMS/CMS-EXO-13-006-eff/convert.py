...
argparser.add_argument(
    "-utilsPath", "--utilsPath", help="path to the package smodels_utils", type=types.StringType
)
argparser.add_argument(
    "-smodelsPath", "--smodelsPath", help="path to the package smodels_utils", type=types.StringType
)
...
info.contact = (
    "Andre Lessa <lessa.a.p@gmail.com>; Jan Heisig <heisig@physik.rwth-aachen.de>; SModelS"
)
...
observedNs = [42, 7, 0, 0]
expectedBGs = [44.0, 5.6, 0.56, 0.02]
bgErrors = [9.0, 1.1, 0.11, 0.004]
...
for i, name in enumerate(datasetNames):
    ...
    plane.setSources(
        ...
        dataFiles=["orig/eff_HSCPM1_" + name + ".txt"],
        dataFormats=["txt"],
    )
    # +++++++ next txName block ++++++++++++++
    ...
    plane.setSources(
        dataLabels=["efficiencyMap"],
        dataFiles=["orig/eff_HSCPM3_" + name + ".txt"],
        dataFormats=["txt"],
    )
    # +++++++ next txName block ++++++++++++++
    ...
    HSCPM5.checked = ""
    HSCPM5.constraint = (
        "[[['*'],['*']],[['*'],['*']]]"  ##Here '*' represents any (single) even particle
    )
    ...
    plane.setSources(
        dataLabels=["efficiencyMap"],
        dataFiles=["orig/eff_HSCPM5_" + name + ".txt"],
        dataFormats=["txt"],
    )
    # +++++++ next txName block ++++++++++++++
    ...
    plane.setSources(
        dataLabels=["efficiencyMap"],
        dataFiles=["orig/eff_HSCPM7_" + name + ".txt"],
        dataFormats=["txt"],
    )
    # +++++++ next txName block ++++++++++++++
    ...
    HSCPM8.checked = ""
    HSCPM8.constraint = (
        "[[['*','*']],[['*','*']]]"  ##Here '*','*' represents any pair of even particles
    )
    ...
    plane.setSources(
        dataLabels=["efficiencyMap"],
        dataFiles=["orig/eff_HSCPM8_" + name + ".txt"],
        dataFormats=["txt"],
    )
    # +++++++ next txName block ++++++++++++++
    ...
    plane.setSources(
        dataLabels=["efficiencyMap"],
        dataFiles=["orig/eff_HSCPM2_" + name + ".txt"],
        dataFormats=["txt"],
    )
    # +++++++ next txName block ++++++++++++++
    ...
    # +++++++ next mass plane block ++++++++++++++
    plane = HSCPM4.addMassPlane(
        [["*"], [x, y]]
    )  ##Here ['*'] represents a mass array with any length
    plane.setSources(
        dataLabels=["efficiencyMap"],
        dataFiles=["orig/eff_HSCPM4_" + name + ".txt"],
        dataFormats=["txt"],
    )
    # +++++++ next txName block ++++++++++++++
    ...
    HSCPM6.checked = ""
    HSCPM6.constraint = (
        "[[*],[['*'],[ '*']]]"  ##Here [*] represents one branch with any list of vertices
    )
    ...
    # +++++++ next mass plane block ++++++++++++++
    plane = HSCPM6.addMassPlane(
        [["*"], [x, y, z]]
    )  ##Here ['*'] represents a mass array with any length
    plane.setSources(
        dataLabels=["efficiencyMap"],
        dataFiles=["orig/eff_HSCPM6_" + name + ".txt"],
        dataFormats=["txt"],
    )
