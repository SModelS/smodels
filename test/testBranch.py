...
sq1 = Particle(Z2parity=-1, mass=150.0 * GeV, pdg=1000005, totalwidth=1 * GeV)
sq2 = Particle(
    Z2parity=-1, label="sq2", mass=100.0 * GeV, pdg=2000005, colordim=3, totalwidth=1 * GeV
)
sq2b = Particle(
    Z2parity=-1, label="sq2b", mass=100.0 * GeV, pdg=1000004, colordim=3, totalwidth=1 * GeV
)
sn1 = Particle(
    Z2parity=-1, label="neutralino1", mass=50.0 * GeV, pdg=1000022, width=0.0 * GeV, decays=[]
)
sn1B = Particle(
    Z2parity=-1, label="neutralino1", pdg=1000022, eCharge=0, colordim=0, totalwidth=0.0 * GeV
)
u = Particle(Z2parity=1, label="q", pdg=3, totalwidth=0.0 * GeV, eCharge=2.0 / 3.0)
e = Particle(
    Z2parity=1,
    label="e-",
    pdg=11,
...
g_decays = [
    pyslha.Decay(br=0.3, nda=3, ids=[-1, 1, 1000022], parentid=1000021),
...
g = Particle(mass=500.0 * GeV, pdg=1000021, Z2Parity=-1, totalwidth=1 * GeV, decays=g_decays)
...
class BranchTest(unittest.TestCase):
    ...
    def testBranchComp(self):
        ...
        N1 = Particle(mass=705.0 * GeV, pdg=1000022, Z2Parity=-1, eCharge=0, colordim=1)
    ...
    def testDecayBranches(self):
        ...
        self.assertEqual(dList[0].maxWeight, 0.6)
