...
def main():
    ...
    parser.add_argument(
        "-v",
        "--verbose",
        help="verbosity level. " "accepted values are: debug, info, warning, error.",
        default="info",
        type=str,
    )
    ...
    xseccomputer = subparsers.add_parser(
        "xseccomputer", description="Compute MSSM cross sections for a SLHA file."
    )
    ...
    xseccomputer.add_argument(
        "-e", "--nevents", type=int, default=10000, help="number of events to be simulated [10000]."
    )
    xseccomputer.add_argument(
        "-v",
        "--verbosity",
    ...
    xseccomputer.add_argument("-C", "--colors", action="store_true", help="colored terminal output")
    xseccomputer.add_argument(
        "--noautocompile", action="store_true", help="turn off automatic compilation"
    )
    xseccomputer.add_argument(
        "-k", "--keep", action="store_true", help="do not unlink temporary directory"
    )
    xseccomputer.add_argument(
        "-6", "--pythia6", action="store_true", help="use pythia6 for LO cross sections"
    )
    xseccomputer.add_argument(
        "-8", "--pythia8", action="store_true", help="use pythia8 for LO cross sections (default)"
    )
    xseccomputer.add_argument(
        "-n", "--NLO", action="store_true", help="compute at the NLO level (default is LO)"
    )
    xseccomputer.add_argument(
        "-N",
        "--NLL",
        help="compute at the NLO+NLL level (takes precedence over NLO, default is LO)",
        action="store_true",
    )
    xseccomputer.add_argument(
        "-O",
        "--LOfromSLHA",
        help="use LO cross sections from file to compute the NLO or NLL cross sections",
        action="store_true",
    )
    xseccomputer.add_argument(
        "-S",
    ...
    slhachecker.add_argument(
        "-xS", "--xsec", help="turn off the check for xsection blocks", action="store_false"
    )
    slhachecker.add_argument(
        "-illegal",
        "--illegal",
    ...
    dbBrowser.add_argument(
        "-p", "--path_to_database", help="path to SModelS database", required=True
    )
    dbBrowser.add_argument(
        "-t",
        "--text",
    ...
    iPlots = subparsers.add_parser(
        "interactive-plots",
        description="Produces a set of interactive plots for visualizing results from a scan.",
    )
    iPlots.add_argument(
        "-p",
        "--parameters",
    ...
    iPlots.add_argument(
        "-f",
        "--smodelsFolder",
        help="path to the smodels folder or tarball (.tar.gz) with the SModelS python output files.",
        required=True,
    )
    iPlots.add_argument(
        "-s",
        "--slhaFolder",
        help="path to the SLHA folder or tarball (.tar.gz) with the SLHA input files.",
        required=True,
    )
    iPlots.add_argument(
        "-o",
    ...
    proxydb = subparsers.add_parser("proxydb", description="create proxy databases for network use")
    proxydb.add_argument(
        "-i", "--inputfile", help="input database file [db31.pcl]", type=str, default="db31.pcl"
    )
    proxydb.add_argument(
        "-s", "--servername", help="the server name [None]", type=str, default=None
    )
    proxydb.add_argument(
        "-v", "--verbose", help="verbosity, for server and client [info]", type=str, default="info"
    )
    proxydb.add_argument("-R", "--rundir", help="the rundir [./]", type=str, default="./")
    proxydb.add_argument(
        "-p", "--serverport", help="the server port [31770]", type=int, default=None
    )
    proxydb.add_argument(
        "-l", "--symlink", help="set a symlink from outputfile to default.pcl", action="store_true"
    )
    proxydb.add_argument(
        "-o",
        "--outputfile",
    ...
    proxydb.add_argument("-r", "--run", help="start the server upon creation", action="store_true")

    toolbox = subparsers.add_parser(
        "toolbox", description="Facility to control external dependencies"
    )
    toolbox.add_argument("-c", "--colors", help="turn on terminal colors", action="store_true")
    toolbox.add_argument("-l", "--long", help="long output lines", action="store_true")
    toolbox.add_argument("-m", "--make", help="compile packages if needed", action="store_true")
