...
class XSecTest(unittest.TestCase):
    ...
    def testLOGlu(self):
        ...
        self.assertAlmostEqual(w8lo / 264.0, 1.0, 2)
    ...
    def testNLLGlu(self):
        ...
        self.assertAlmostEqual(w8nll / 573.0, 1.0, 2)
    ...
    def testLOGlu13(self):
        ...
        self.assertAlmostEqual(w13lo / 2237.0, 1.0, 1)
    ...
    def testNLLGlu13(self):
        ...
        self.assertAlmostEqual(w13nll / 4320.0, 1.0, 2)
    ...
    def testXSecMain(self):
        ...
        self.assertAlmostEqual(lo / 264.0, 1.0, 2)
        self.assertAlmostEqual(nll / 573.6, 1.0, 2)
        # Check 13 TeV xsecs:
        lo = xsecsInfile.getXsecsFor("13 TeV (LO)")[0].value.asNumber(fb)
        nll = xsecsInfile.getXsecsFor("13 TeV (NLL)")[0].value.asNumber(fb)
        self.assertAlmostEqual(lo / 2230.0, 1.0, 1)
        self.assertAlmostEqual(nll / 4308.0, 1.0, 1)
    ...
    def testSSMultipliers(self):
        ...
        xargs.filename = tmpfile
        xargs.colors = False
        xargs.ssmultipliers = {(1000021, 1000021): 4.0}
        ...
        lo = xsecsInfile.getXsecsFor("8 TeV (LO)")[0].value.asNumber(fb)
        nll = xsecsInfile.getXsecsFor("8 TeV (NLL)")[0].value.asNumber(fb)
        self.assertAlmostEqual(lo / 1058.444, 1.0, 1)
        self.assertAlmostEqual(nll / 2299.046, 1.0, 1)
        # Check 13 TeV xsecs:
        lo = xsecsInfile.getXsecsFor("13 TeV (LO)")[0].value.asNumber(fb)
        nll = xsecsInfile.getXsecsFor("13 TeV (NLL)")[0].value.asNumber(fb)
        self.assertAlmostEqual(lo / 8910.76, 1.0, 1)
        self.assertAlmostEqual(nll / 17215.5906, 1.0, 1)
    ...
    def testSSJokers(self):
        ...
        xargs.filename = tmpfile
        xargs.colors = False
        xargs.ssmultipliers = {("*100002?", "*1000021"): 4.0}
        # xargs.ssmultipliers = { 1000021: 2. }
        xargs.verbosity = "warning"
        # Compute LO cross sections
        xsecComputer.main(xargs)
        ...
        lo = xsecsInfile.getXsecsFor("8 TeV (LO)")[0].value.asNumber(fb)
        nll = xsecsInfile.getXsecsFor("8 TeV (NLL)")[0].value.asNumber(fb)
        self.assertAlmostEqual(lo / 1056.0, 1.0, 2)
        self.assertAlmostEqual(nll / 2294.0, 1.0, 2)
        # Check 13 TeV xsecs:
        lo = xsecsInfile.getXsecsFor("13 TeV (LO)")[0].value.asNumber(fb)
        nll = xsecsInfile.getXsecsFor("13 TeV (NLL)")[0].value.asNumber(fb)
        self.assertAlmostEqual(lo / 8910.76, 1.0, 2)
        self.assertAlmostEqual(nll / 17215.5906, 1.0, 1)
