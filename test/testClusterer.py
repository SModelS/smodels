...
class ClustererTest(unittest.TestCase):
    def testSimpleCluster(self):
        ...
        data = [
            [[[674.99 * GeV, 199.999 * GeV], [674.99 * GeV, 199.999 * GeV]], 0.03 * fb],
            [[[725.0001 * GeV, 200.0 * GeV], [725.0001 * GeV, 200.0 * GeV]], 0.06 * fb],
            [[[750.0 * GeV, 250.0 * GeV], [750.0 * GeV, 250.0 * GeV]], 0.03 * fb],
        ]
        ...
        txname = TxName(
            "./database/8TeV/ATLAS/ATLAS-SUSY-2013-05/data/T2bb.txt", globalInfo, info, finalStates
        )
        ...
        el1.weight = w1
        el1.txname = txname
        el1.eff = 1.0  # (Used in clustering)
        ...
        el2.branches[0].oddParticles[0].__setattr__("mass", 725.0 * GeV)
        el2.branches[1].oddParticles[0].__setattr__("mass", 725.0 * GeV)
        el2.eff = 1.0  # (Used in clustering)

        # Cluster for upper limits (all elements close in upper limit should be clustered together)
        maxDist = 5.0  # Cluster all elements
        newel = clusterTools.clusterElements([el1, el2], maxDist, dataset)[0]
        newmasses = newel.averageElement().mass
        self.assertEqual(newmasses, [[700.0 * GeV, 200.0 * GeV]] * 2)
    ...
    def testClusteringEM(self):
        ...
        dataset = database.getExpResults(
            analysisIDs="CMS-SUS-13-012", datasetIDs="3NJet6_800HT1000_300MHT450"
        )[0].getDataset("3NJet6_800HT1000_300MHT450")
        ...
        el1.eff = 1.0  # (Used in clustering)
        el2.eff = 1.0  # (Used in clustering)
        el3.eff = 1.0  # (Used in clustering)
        # All elements have the same UL (for EM results)
        el1._upperLimit = el2._upperLimit = el3._upperLimit = 1.0 * fb
        # Clustering should not depend on the mass, width or txname:
        el1.txname = el2.txname = el3.txname = None
        el1.mass = [[1000.0 * GeV, 10 * GeV]] * 2
        el2.mass = [[1500.0 * GeV, 10 * GeV]] * 2
        el3.mass = [[200.0 * GeV, 100 * GeV, 90.0 * GeV]] * 2
        el3.totalwidth = [[1e-10 * GeV, 1e-15 * GeV, 0.0 * GeV]] * 2
    ...
    def testClusteringUL(self):
        ...
        dataset = database.getExpResults(analysisIDs="ATLAS-SUSY-2013-02", datasetIDs=None)[
            0
        ].getDataset(None)
        ...
        el1.mass = [[1000.0 * GeV, 100.0 * GeV]] * 2
        el2.mass = [[1020.0 * GeV, 100.0 * GeV]] * 2
        el3.mass = [[500.0 * GeV, 100.0 * GeV]] * 2
        ...
        averageMasses = [
            [
                [
                    (1000.0 * GeV * weights[0] + 1020.0 * GeV * weights[1])
                    / (weights[0] + weights[1]),
                    100.0 * GeV,
                ]
            ]
            ...
            for ibr, br in enumerate(avgEl.mass):
                for im, m in enumerate(br):
                    self.assertAlmostEqual(
                        m.asNumber(GeV), averageMasses[ic][ibr][im].asNumber(GeV)
                    )
            self.assertEqual(avgEl.totalwidth, elClusters[ic][0].totalwidth)

        # Check clustering with distinct elements but no maxDist limit
        clusters = clusterElements([el1, el2, el3], maxDist=10.0, dataset=dataset)
        self.assertEqual(len(clusters), 1)
        cluster = clusters[0]
        avgEl = cluster.averageElement()
        averageMass = [
            [
                (1000.0 * GeV * weights[0] + 1020.0 * GeV * weights[1] + 500.0 * GeV * weights[2])
                / sum(weights),
                100.0 * GeV,
            ]
        ] * 2
        ...
        el1._upperLimit = 1.0 * fb
        el2._upperLimit = 1.0 * fb
        el3._upperLimit = 1.0 * fb
    ...
    def testClustererLifeTimes(self):
        ...
        el2.branches[0].oddParticles[0].__setattr__("mass", 675.0 * GeV)
        el2.branches[1].oddParticles[0].__setattr__("mass", 675.0 * GeV)
        ...
        newel = clusterTools.clusterElements([el1, el2], 5.0, dataset)
        ## this example gives an avg cluster mass of 700 gev
        self.assertEqual(newel[0].averageElement().mass[0][0], 675.0 * GeV)
        self.assertAlmostEqual(
            newel[0].averageElement().totalwidth[0][0].asNumber(GeV) * 1e15, 0.95
        )

        newel = clusterTools.clusterElements([el1, el2], 0.5, dataset)
        # in this example the distance is in maxdist, so we cluster
        self.assertTrue(len(newel) == 1)

        newel = clusterTools.clusterElements([el1, el2], 0.1, dataset)
        # in this example the distance is not in maxdist, so we dont cluster
        self.assertTrue(len(newel) == 2)
    ...
    def testComplexCluster(self):
        ...
        predictions = theoryPredictionsFor(
            expResult, toplist, combinedResults=False, marginalize=False
        )
