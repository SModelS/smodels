...
class DatabaseClient:
    def __init__(
        ...
        self.pprint(
            "connecting to %s port %s, rundir is %s. server status is '%s'"
        ...
        try:
            ...
            while self.ntries < self.maxtries:
                try:
                    ...
                    self.ntries += 1
                    self.log(
                        "could not connect to %s. trying again in %d seconds"
                        % (self.nameAndPort(), dt)
                    )
                    time.sleep(dt)
    ...
    def initialize(self):
        ...
        while self.ntries < self.maxtries:
            try:
                ...
                self.ntries += 1
                self.log(
                    "could not connect to %s after %d times. trying again in %d seconds"
        ...
        raise SModelSError(
            "Could not connect to database in initialize, tried %d times" % self.ntries
        )
...
def stresstest(args):
    ...
    client = DatabaseClient(
        servername, port, verbose=verbosity, logfile="@@rundir@@/dbclient%d.log" % nr, clientid=nr
    )
...
if __name__ == "__main__":
    ...
    argparser = argparse.ArgumentParser(description="an instance of a database client")
    argparser.add_argument(
        "-s", "--servername", help="The server name [None]", type=str, default=None
    )
    argparser.add_argument("-v", "--verbosity", help="Verbosity [info]", type=str, default="info")
    argparser.add_argument("-R", "--rundir", help="run directory [./]", type=str, default="./")
    argparser.add_argument(
        "-x", "--shutdown", help="Send shutdown command to server", action="store_true"
    )
    argparser.add_argument(
        "--stresstest", help="stress test the server, send a lot of requests", action="store_true"
    )
    argparser.add_argument(
        "-q",
        "--query",
        ...
        type=str,
        default="obs:ATLAS-SUSY-2017-01:SRHad-Low:TChiWH:[[500,100],[500,100]]",
    )
    argparser.add_argument("-p", "--port", help="port to listen to [31770]", type=int, default=None)
    ...
    if args.stresstest:
        ...
        sys.exit()
    client = DatabaseClient(
        args.servername, args.port, args.verbosity, rundir=args.rundir, clientid=0
    )
    client.initialize()
    if args.shutdown:
