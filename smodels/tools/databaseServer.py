...
class DatabaseServer:
    ...
    def lookUpResult(self, data):
        ...
        if tokens[0] == "exp":
            expected = True
        self.log("looking up for %s,%s,%s,%s" % (anaId, dType, txname, massv))
            ...
            for ds in exp.datasets:
                ...
                for txn in ds.txnameList:
                    if txn.txName != txname:
                        continue
                    coords = txn.txnameData.dataToCoordinates(
                        massv, txn.txnameData._V, txn.txnameData.delta_x
                    )
                    res = None
                    if expected:
        ...
        try:
            ...
            while True:
                ...
                if data:
                    ...
                    self.log(
                        "no more data from %s:%s" % (self.client_address[0], self.client_address[1])
                    )
                    break
        finally:
    ...
    def initialize(self):
        ...
        self.pprint("I will be serving database %s at %s" % (self.db.databaseVersion, self.dbpath))
        try:
            self.sock.bind(self.server_address)
        except OSError as e:
            self.pprint("exception %s. is host ''%s'' reachable?" % (e, self.server_address))
            sys.exit(-1)
...
if __name__ == "__main__":
    ...
    argparser = argparse.ArgumentParser(description="an instance of a database server")
    argparser.add_argument(
        "-d",
        "--dbpath",
        help="The database path [./original.pcl]",
        type=str,
        default="./original.pcl",
    )
    argparser.add_argument("-R", "--rundir", help="The rundir [./]", type=str, default="./")
    argparser.add_argument("-p", "--port", help="port to listen to [31770]", type=int, default=None)
    argparser.add_argument("-v", "--verbose", help="verbosity [info]", type=str, default="info")
    argparser.add_argument(
        "-s",
        "--servername",
    ...
    server = DatabaseServer(args.dbpath, args.servername, args.port, args.verbose, args.rundir)
    server.run()
