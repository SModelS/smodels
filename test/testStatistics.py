...
class StatisticsTest(unittest.TestCase):
    ...
    def testUnderfluctuatingLlhdsFromLimits(self):
        ...
        for nsig in [0, 5]:
            for allowNegatives in [False, True]:
                llhdlim, muhat, sigma_exp = likelihoodFromLimits(4.5, 5.45, nsig, False, allowNegatives)
    ...
    def testCorrectedLlhdsFromLimits(self):
        ...
        for nsig in [0, 3, 5]:
            for x in [0., 0.6]:
                llhdlim,muhat,sigma_exp = likelihoodFromLimits(8.52, 6.18, nsig, False, False,
                          corr=x)
    ...
    def testApproxGaussian(self):
        ...
        import numpy
        c = 0.
        for muval in numpy.arange(0., .2, .02):
