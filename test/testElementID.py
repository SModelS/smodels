...
class ElementIdTest(unittest.TestCase):
    def testGoodFile(self):
        ...
        listOfIDs = {
            "ATLAS-CONF-2013-037": [31 + 8, 26 + 8, 33 + 8, 34 + 8, 27 + 8, 28 + 8, 30 + 8, 32 + 8],
        ...
        )  # Make sure C1 and N2 are treated as prompt
        ## model.describe()

        topoList = decomposer.decompose(
            model, sigmacut=0.1 * fb, doCompress=True, doInvisible=True, minmassgap=5 * GeV
        )
        resultlist = database.getExpResults(
            analysisIDs=["*:8*TeV", "CMS-PAS-SUS-15-002", "CMS-PAS-SUS-16-024"]
        )

        for res in resultlist:
            if "EXO" in res.globalInfo.id:
                continue  # skip EXO searches
