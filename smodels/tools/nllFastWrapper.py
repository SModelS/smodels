...
class NllFastWrapper(WrapperBase):
    ...
    def _compute(self, energy, pIDs, pdf, squarkmass, gluinomass):
        ...
        if process == "st":
            ...
            nll_run = (
                "./nllfast_" + energy + " %s %s %s %s" % (process, pdf, squarkmass, gluinomass)
            )
    ...
    def _getProcessName(self, pIDs):
        ...
        if pid1 in antisquarks and pid2 in squarks:
            ...
            process = "gg"
        elif abs(pid1) in squarks and pid2 == 1000021 or abs(pid2) in squarks and pid1 == 1000021:
            process = "sg"
    ...
    def _getDecoupledKfactors(self, process, energy, pdf, mass):
        ...
        if process != "sb" and process != "gg":
            return None
        elif process == "sb":
            process_dcpl = "sdcpl"
        elif process == "gg":
            process_dcpl = "gdcpl"
        nll_run = "./nllfast_" + energy + " %s %s %s" % (process_dcpl, pdf, mass)
    ...
    def getKfactorsFor(self, pIDs, slhafile, pdf="cteq"):
        ...
        for check in check_pids:
            if not check in masses.entries:
                logger.error(
                    "cannot compute k factor for pdgid %d: "
                    " no particle mass given. will set mass to inf." % check
                )
                masses.entries[check] = 1.0e10

        gluinomass = abs(masses.entries[1000021])
        squarkmass = sum([abs(masses.entries[pid]) for pid in squarks]) / 8.0
        pid1, pid2 = sorted(pIDs)
        if pid1 in antisquarks and pid2 in squarks:
            squarkmass = (abs(masses.entries[abs(pid1)]) + abs(masses.entries[pid2])) / 2.0
        elif pid1 in squarks and pid2 in squarks:
            squarkmass = (abs(masses.entries[pid1]) + abs(masses.entries[pid2])) / 2.0
        elif abs(pid1) == pid2 and pid2 in third:
        ...
        if "too low/high gluino" in nll_output.lower():
            if gluinomass > 500.0 and process == "sb":
                doDecoupling = True
                dcpl_mass = gluinomass
        elif "too low/high squark" in nll_output.lower():
            if squarkmass > 500.0 and process == "gg":
                doDecoupling = True
                dcpl_mass = squarkmass

        # If process do not have decoupled grids, return None:
        if not doDecoupling:
            if gluinomass < 99000.0 and squarkmass < 99000.0:
                ## dont warn about obviously decoupled cases
                logger.warning(
                    "Masses of (q,g)=(%s,%s) out of NLLfast grid for %s, %s"
                    % (squarkmass, gluinomass, process, energy)
                )
            return (None, None)
        ...
        if not kfacs:
            logger.warning(
                "Error obtaining k-factors from the NLLfast decoupled grid for " + process
            )
            return (None, None)
        elif dcpl_mass / min(gluinomass, squarkmass) > 10.0:
            return kfacs
        # Interpolate between the non-decoupled and decoupled grids
        else:
            kFacsVector = [
                [10.0 * min(gluinomass, squarkmass), kfacs]
            ]  # First point for interpolation (decoupled grid)
            kfacs = None
            while not kfacs and dcpl_mass > 500.0:
                dcpl_mass -= 100.0  # Reduce decoupled mass, until NLLfast produces results
                if process == "sb":
            ...
            kFacsVector.append(
                [dcpl_mass, kfacs]
            )  # Second point for interpolation (non-decoupled grid)
        ...
        if len(kFacsVector) < 2:
            ...
            kFacs = self._interpolateKfactors(kFacsVector, max(squarkmass, gluinomass))
        return kFacs
...
class NllFastWrapper7(NllFastWrapper):
    ...
    def __init__(self):
        NllFastWrapper.__init__(
            self,
            7,
            "1.2",
            testParams="gg cteq 500 600",
            testCondition="500.     600.    0.193E+00  " "0.450E+00  0.497E+00",
        )
...
class NllFastWrapper8(NllFastWrapper):
    ...
    def __init__(self):
        NllFastWrapper.__init__(
            ...
            "2.1",
            testParams="gg cteq 500 600",
            testCondition="500.     600.    0.406E+00  " "0.873E+00  0.953E+00",
        )
...
class NllFastWrapper13(NllFastWrapper):
    ...
    def __init__(self):
        NllFastWrapper.__init__(
            ...
            "3.1",
            testParams="gg cteq 500 600",
            testCondition="600.    0.394E+01  0.690E+01  " "0.731E+01    0.394E+00",
        )


nllFastTools = {7: NllFastWrapper7(), 8: NllFastWrapper8(), 13: NllFastWrapper13()}
