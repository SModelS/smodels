    ...
    for series in alltests:
        for test in series:
            if type(test) != unittest.suite.TestSuite:
                print("%sError: could not import ``%s'' {%s}" % (colors.error, test, type(test)))
            ...
            for t in test:
                ...
                try:
                    ...
                    n_failed += 1
                    print("%s FAILED: %s,%s%s" % (colors.error, type(e), str(e), colors.reset))
                    continue
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "-c", "--clean_database", help="remove database pickle files", action="store_true"
    )
    ap.add_argument("-v", "--verbose", help="run verbosely", action="store_true")
    ap.add_argument(
        "-f",
        "--filter",
        help="run only tests that have <FILTER> in name. Works only with verbose and not parallel. case sensitive.",
        type=str,
        default=None,
    )
