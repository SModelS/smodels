    ...
    for expResult in database.expResultList:
        ...
        for theoPred in expResults:
            ...
            if hasattr(expResult.globalInfo, "regionMappings"):
                assert type ( expResult.globalInfo.regionMappings) == dict, f"{expResult.globalInfo.id} srMappings are not a dict"
...
def _isDatasetInCombination ( dataset, expResult ) -> Union[None,bool]:
    ...
    if not hasattr ( expResult.globalInfo, "srSets" ):
        raise SModelSError ( f"{expResult.globalInfo.id} has regionMappings but no srSets" )
    # if dataId not in expResult.globalInfo.regionMappings:  ## [!AL!] dataId refers to the smodels name, no? So it should not be in a key of srMappings, which corresponds to the label
        # return False
...
def _getCombinedResultFor(dataSetResults, expResult):
    ...
    for predList in dataSetResults:
        for tpred in predList:
            ...
            if hasattr ( globalInfo, "regionMappings" ): ## [!AL!] dataId refers to the smodels name, no? So it should not be in a key of srMappings, which corresponds to the label
